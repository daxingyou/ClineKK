...
function TableLogic:init()
	...
	luaPrint("PLAY_COUNTinit:",PLAY_COUNT);
	for i=1,PLAY_COUNT do
		table.insert(self._playerSitted, false);
	end
	luaDump(self._playerSitted, "self._playerSitted----1", 5)
end
...
function TableLogic:loadUsers()
	...
	self:loadDeskUsers();
	luaPrint("loadUsers_PLAY_COUNT",PLAY_COUNT)
	for i=1,PLAY_COUNT do
    	local k = i -1;
	    ...
	    if self._existPlayer[i] == true and user ~= nil then
			...
			luaPrint("loadUsers_viewseatNo-----------:",seatNo,k)
			self._callback:loadUser(seatNo, k == self._mySeatNo);
...
function TableLogic:dealUserSitResp(userSit, user)
    ...
    if isMe then
        ...
        if self._callback.enterUser then
        	self._callback:enterUser(seatNo,userSit.bDeskStation == self._mySeatNo)
        else
        	luaPrint("self._callback.enterUser___________is_nil")
        end
...
function TableLogic:dealUserUpResp(userUp,user,bIsMe)
	...
	-- self._callback:removeUser(seatNo, bIsMe);
	-- self._callback:playerGetReady(seatNo, false);
end
...
function TableLogic:dealGameStationResp(object, objectSize)
	...
	luaPrint("dealGameStationResp_self._gameStatus:",self._gameStatus)
	local msg = {}
	if self._gameStatus == GameMsg.GS_WAIT_SETGAME then 	--GS_WAIT_SETGAME 0
		msg = convertToLua(object,GameMsg.GameStationFree);
		msg.id = GameMsg.MSG_TABLE_FREE;
		self._callback:onGameStationWait(msg);
	elseif self._gameStatus == GameMsg.GS_WAIT_ARGEE then 	--GS_WAIT_ARGEE 1
		msg = convertToLua(object,GameMsg.GameStationFree);
		msg.id = GameMsg.MSG_TABLE_FREE;
		self._callback:onGameStationWait(msg);
	elseif self._gameStatus == GameMsg.GS_SEND_CARD then 	--21,	--发牌状态
		msg = convertToLua(object,GameMsg.GameStationBegin);
		msg.id = GameMsg.MSG_TABLE_SEND;
		self._callback:onGameStationSend(msg);
	elseif self._gameStatus == GameMsg.GS_PLAY_GAME then      --22,	--游戏中状态
		msg = convertToLua(object,GameMsg.GameStation_PLAYING);
		msg.id = GameMsg.MSG_TABLE_PLAYING;
		self._callback:onGameStationPlaying(msg);
		
	elseif self._gameStatus == GameMsg.GS_WAIT_NEXT then 	--23--等待下一盘开始
		msg = convertToLua(object,GameMsg.GameStationFree);
		msg.id = GameMsg.MSG_TABLE_FREE;
		self._callback:onGameStationWait(msg);
	...
	luaDump(msg,"场景消息_dealGameStationResp");
...
function TableLogic:dealUserAgreeResp(agree)
	...
	if 1 == agree.bAgreeGame then
		...
		if self._callback.onReadyMsg ~= nil then
			self._callback:onReadyMsg(seatNo, true);
		else
			luaPrint("self._callback:onReadyMsg_____________-nil")
		end
...
function TableLogic:sendForceQuit(source)
	...
	self._callback:leaveDeskSsz(source);
end
...
--下注
function TableLogic:sendBet(nChip)
	...
	local msg = {};
	msg.iVrebType = GameMsg.TYPE_NOTE;
	msg.iNote = nChip;
	msg.bMing = false;
	msg.byComparedStation = 255;

	luaPrint("szp_sendBet PlatformLogic.loginResult.nNameID:",PlatformLogic.loginResult.dwUserID);
	luaDump(msg, "msg_sendbet", 5)

	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY, GameMsg.ASS_VREB_CHECK, msg, GameMsg.Cr_UserProcess);
end

--跟注
function TableLogic:sendFollowBet(nChip)
	local msg = {};
	msg.iVrebType = GameMsg.TYPE_FOLLOW;
	msg.iNote = nChip;
	msg.bMing = false;
	msg.byComparedStation = 255;

	luaPrint("szp_sendFollowBet PlatformLogic.loginResult.nNameID:",PlatformLogic.loginResult.dwUserID);
	luaDump(msg, "msg_sendFollowBet", 5)
...
--加注
function TableLogic:sendRasieBet(nChip)
	local msg = {};
	msg.iVrebType = GameMsg.TYPE_ADD;
	msg.iNote = nChip;
	
	msg.byComparedStation = 255;

	luaPrint("szp_sendRasieBet PlatformLogic.loginResult.nNameID:",PlatformLogic.loginResult.dwUserID);
	luaDump(msg, "sendRasieBet", 5)

	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY, GameMsg.ASS_VREB_CHECK, msg, GameMsg.Cr_UserProcess);
end

--看牌
function TableLogic:sendCheck()
	local msg = {};
	msg.iVrebType = GameMsg.TYPE_LOOKCARD;
	msg.iNote = 0;
	
	msg.byComparedStation = 255;

	luaPrint("szp_sendCheck PlatformLogic.loginResult.nNameID:",PlatformLogic.loginResult.dwUserID);
	luaDump(msg, "sendCheck", 5)
...
--请求比牌
function TableLogic:sendReqCompare()
	local msg = {};
	msg.iVrebType = GameMsg.TYPE_COMPARE_REQ;
	msg.iNote = 0;
	
	msg.byComparedStation = 255;

	luaPrint("szp_sendCompReq PlatformLogic.loginResult.nNameID:",PlatformLogic.loginResult.dwUserID);
	luaDump(msg, "sendCompReq", 5)
...
--比牌
function TableLogic:sendCompare(tSeat)
	local msg = {};
	msg.iVrebType = GameMsg.TYPE_COMPARE_CARD;
	msg.iNote = 0;
	
	msg.byComparedStation = tSeat;

	luaPrint("szp_sendCompare PlatformLogic.loginResult.nNameID:",PlatformLogic.loginResult.dwUserID);
	luaDump(msg, "sendCompare", 5)
...
--全下
function TableLogic:sendAllIn()
	local msg = {};
	msg.iVrebType = GameMsg.TYPE_NOTE_ALL;
	msg.iNote = 0;
	
	msg.byComparedStation = 255;

	luaPrint("szp_sendAllIn PlatformLogic.loginResult.nNameID:",PlatformLogic.loginResult.dwUserID);
	luaDump(msg, "sendAllIn", 5)
...
--弃牌
function TableLogic:sendFold()
	local msg = {};
	msg.iVrebType = GameMsg.TYPE_GIVE_UP;
	msg.iNote = 0;
	
	msg.byComparedStation = 255;

	luaPrint("szp_sendFold PlatformLogic.loginResult.nNameID:",PlatformLogic.loginResult.dwUserID);
	luaDump(msg, "sendFold", 5)
...
--亮牌
function TableLogic:sendEveal()
	local msg = {};
	msg.iVrebType = GameMsg.TYPE_MINGCARD;
	msg.iNote = 0;
	
	msg.byComparedStation = 255;

	luaPrint("szp_sendEveal PlatformLogic.loginResult.nNameID:",PlatformLogic.loginResult.dwUserID);
	luaDump(msg, "sendEveal", 5)
...
--自动准备
function TableLogic:sendAutoFollow(bAutoFollow)
	local msg = {};
	msg.bAutoFollow = bAutoFollow;

	luaDump(msg, "sendAutoFollow", 5)

	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY, GameMsg.ASS_AUTO_FOLLOW, msg, GameMsg.AutoFollow);
end
