...
function CreateGoldRoomLayer:initUI()
	...
	-- listView:setContentSize(cc.size(winSize.width, mask:getContentSize().height))
	-- listView:setPosition(mask:getContentSize().width*0.5,mask:getContentSize().height*0.5)
	...
	for i=2,4 do -- 3
		local k = i-1;
		local layout = self:createRoomGroup(i,4)
		layout:setAnchorPoint(0.5,0.5);
		layout:setPosition(size.width/2+(k-2)*layout:getContentSize().width,size.height/2);
...
function CreateGoldRoomLayer:createRoomGroup(groupId,num)
	...
	local layout = ccui.Layout:create()
	layout:setContentSize(cc.size(380,480))
...
function CreateGoldRoomLayer:createItem(tag)
	...
	if difenNum[tag] then
		...
		local maxzhu = FontConfig.createWithSystemFont("最大下注: "..difenNum[tag],28,cc.c3b(255,225,146));
		maxzhu:setPosition(size.width/2,size.height/2-135)
		btn:addChild(maxzhu)
	else
		-- btn:setTag(#difenNum)
...
function CreateGoldRoomLayer:getRootIndex()
	local roomid = mRoomId[1];

	for k,v in pairs(self.roomList) do
		local findFlag = false;

		for k1,roomid in pairs(mRoomId) do
			if roomid == v.uRoomID then
				findFlag = true;
				break;
			end
		end
...
function CreateGoldRoomLayer:enterGame(sender)
	if type(sender) == "number" then
		self.m_selectedRoomID = sender
	else
		self.m_selectedRoomID = sender:getTag()
	end	
...
function CreateGoldRoomLayer:onClick(sender)
	...
	if tag == 1 then
		...
		local layer = require("superfruitgame.HelpLayer"):create();
