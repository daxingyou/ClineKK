...
function GameDeskLayer:ctor(maxCount)
	self.super.ctor(self,0,false)

	self:initData(maxCount)
...
function GameDeskLayer:initData(maxCount)
	...
	self.deskCount = 0;
	self.deskInfo = {};
	self.MaxCount = maxCount or globalUnit.iPlayCount;  --游戏最大人数
...
function GameDeskLayer:initUI()
	...
	listView:setContentSize(cc.size(900, 390))
	listView:setPosition(size.width/2+130,size.height*0.4+70)
	self:addChild(listView)
	self.listView = listView

	local guatiao = ccui.ImageView:create("newExtend/vip/common/di2.png")
    guatiao:setPosition(size.width*0.5+450,size.height*0.19-3)
    ...
    local Label_name = FontConfig.createWithSystemFont(gameName..msg,26 ,cc.c3b(90, 44, 14));
    Label_name:setPosition(75,guatiao:getContentSize().height/2-10)
	...
	btn:setPosition(105+size.width/2, 7)
...
function GameDeskLayer:createDeskList(row)
	...
	for i=1,3 do
		if index <= self.deskCount then
			...
			local node = require("shisanzhanglq.DeskNode"):create(index,self.deskInfo,self.roomInfo.uRoomID,self.MaxCount)
