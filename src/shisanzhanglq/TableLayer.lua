...
local Player = require("shisanzhanglq.Player")
local TableLogic = require("shisanzhanglq.TableLogic")
local Audio = require("shisanzhanglq.Audio")
local CardListBoard = require("shisanzhanglq.CardListBoard")
...
function TableLayer:onEnter()
	...
	cc.SpriteFrameCache:getInstance():addSpriteFrames("shisanzhanglq/cards/cards.plist")
...
function TableLayer:bindEvent()
	self:pushEventInfo(shisanzhanglqInfo, "gameStartInfo", handler(self, self.receiveGameStartInfo))        		--游戏开始
	self:pushEventInfo(shisanzhanglqInfo, "sendCardInfo", handler(self, self.receiveSendCardInfo))  				--发牌
	self:pushEventInfo(shisanzhanglqInfo, "gameResultInfo", handler(self, self.receiveGameResultInfo))            --比牌结果
	self:pushEventInfo(shisanzhanglqInfo, "gameEndInfo", handler(self, self.receiveGameEndInfo))        			--游戏结束
	self:pushEventInfo(shisanzhanglqInfo, "openCardInfo", handler(self, self.receiveOpenCardInfo))           		--摊牌
...
function TableLayer:loadLayout()
	...
	self._btnGZ:onClick(function()
		local HelpLayer = require("shisanzhanglq.HelpLayer")
	...
	if globalUnit.isShowZJ then
		self._btnGZ:loadTextures("shisanzhanglq/zhanji/paixing.png","shisanzhanglq/zhanji/paixing-on.png")
		self._btnMore:loadTextures("shisanzhanglq/zhanji/gengduo-shisanzhang.png","shisanzhanglq/zhanji/gengduo-shisanzhang-on.png")
		self._imgMore:loadTexture("shisanzhanglq/zhanji/xialadiban.png")
		self._imgMore:setContentSize(cc.size(96,234))
		self._imgMore:setPositionY(self._imgMore:getPositionY()-30)

		local btn = ccui.Button:create("shisanzhanglq/zhanji/zhanji.png","shisanzhanglq/zhanji/zhanji-on.png");
...
function TableLayer:showDeskInfoGold(nBasePoint, nMoneyLimit)
	...
	if self.difen=="1.00" then
		self.ruchang = 50
	elseif self.difen=="5.00" then
		self.ruchang = 250
	elseif self.difen=="10.00" then
		self.ruchang = 500
	elseif self.difen=="20.00" then
		self.ruchang = 1000
	else
		self.ruchang = nMoneyLimit
	end
...
function TableLayer:showBiPaiFinish(seatNo, visible)
	...
	if self._readyBiPai[seatNo] then
		...
		if visible then
			-- self._cardListBoard[seatNo]:setVisible(false)
		end
	end
end
...
function TableLayer:onClickExitGameCallBack()
	local func = function()
		if self.isPlaying or globalUnit.nowTingId > 0 then
...
function TableLayer:showGameStatus(obj, status)
	...
	if status == GameMsg.GS_TK_FREE then
		...
		if globalUnit.nowTingId > 0 then
			-- for i=1,PLAY_COUNT do
			-- 	local viewSeat = self.tableLogic:logicToViewSeatNo(i);
			-- 	self:playerGetReady(viewSeat, data.bAgree[i])
			-- 	if i-1 == self.tableLogic._mySeatNo then
			-- 		self._btnStart:setVisible(not data.bAgree[i])
			-- 	end
			-- end
		end
	-- 游戏中
...
function TableLayer:showGameResultView()
	...
	self._btnHZ:onClick(function(sender)
		...
		local score = tonumber(self._players[1]._money)
		local cellScore = self.difen

		if score < self.ruchang then
			local prompt = GamePromptLayer:create();
			prompt:showPrompt(GBKToUtf8("最低需要"..self.ruchang.."金币以上！"));
        ...
        local shisanzhanglq = require("shisanzhanglq");
        shisanzhanglq.reStart = true;
        UserInfoModule:clear();

		Hall:exitGame(1,function() 
            self:leaveDesk(1);
            shisanzhanglq:ReSetTableLayer(score,cellScore); 
        end);
	end)
...
function TableLayer:ChangeOldLayer(layer)
    ...
    local winSize = cc.Director:getInstance():getWinSize();
    local enterText = cc.Sprite:create("shisanzhanglq/bg/ddz_fangjian.png");
...
function TableLayer:startClock(layer)
    local times = 10
    local winSize = cc.Director:getInstance():getWinSize();
    local daojishi = FontConfig.createWithCharMap(tostring(times), "shisanzhanglq/number/ddz_daojishizitiao.png", 34, 42, "+");
...
function TableLayer:cancelMatch(layer)
    local winSize = cc.Director:getInstance():getWinSize();
    local btn = ccui.Button:create("shisanzhanglq/bg/ddz_quxiao.png","shisanzhanglq/bg/ddz_quxiao-on.png")
...
function TableLayer:playerGetReady(byStation, bAgree, isRecur)
    ...
    if globalUnit.nowTingId > 0 then
	  --   if self.readyIMG[byStation] then
	  --   	self.readyIMG[byStation]:setVisible(bAgree); 
	  --   end 
	  --   if byStation == 1 and bAgree then
	  --   	self._btnStart:hide()
	  --   	self:clearUI()
