...
function CreateGoldRoomLayer:initUI()
	...
	local title = ccui.ImageView:create("createRoom/name.png")
	...
	listView:setPosition((1559-winSize.width)/2,120)
	listView:setScrollBarEnabled(false)
	listView:setClippingEnabled(false)
	self.bg:addChild(listView)
	self.listView = listView

	local layoutWidth = (winSize.width - 400*4)/2;
...
function CreateGoldRoomLayer:createRoomGroup(groupId,num)
	...
	local layout = ccui.Layout:create()
	layout:setContentSize(cc.size(400,self.listView:getContentSize().height))
...
function CreateGoldRoomLayer:createItem(tag)
	local difenNum = {nil,"1.00","5.00","10.00","20.00"}
	local ruchang = {nil,"50.00","250.00","500.00","1000.00"}
	...
	if difenNum[tag] then
		local difen = FontConfig.createWithCharMap(difenNum[tag].."底分","game/createRoomduizhan/zitiao1.png",24,24,"+",{{"底分",":;<"}})
		difen:setPosition(size.width/2+10,size.height/2-160)
		btn:addChild(difen)

		local difen = FontConfig.createWithSystemFont(ruchang[tag].."入场",30)
		difen:setPosition(size.width/2,size.height/2-110)
		btn:addChild(difen)
		difen:setColor(cc.c3b(253,245,218));
	else
		btn:setTag(#difenNum)
...
function CreateGoldRoomLayer:enterGame(sender)
	...
	luaPrint(globalUnit.iRoomIndex, "globalUnit.iRoomIndex")
