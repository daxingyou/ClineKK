...
local DeskInfoLayer = require("shisanzhanglq.DeskInfoLayer");
-- //桌子信息
...
function DeskNode:initData()
	if self.MaxCount==4 then
		self.pos = {{0,0.7},{0.5,0.4},{1,0.7},{0.5,1.1}}
	elseif self.MaxCount==6 then
		self.pos = {{0,0.7},{0.3,0.4},{0.7,0.4},{0.3,1.1},{0.7,1.1},{1,0.7}}
	elseif self.MaxCount==8 then
		self.pos = {{0,0.5},{0,0.9},{0.3,0.4},{0.7,0.4},{0.3,1.1},{0.7,1.1},{1,0.5},{1,0.9}}
	end
end
...
function DeskNode:initUI()
	...
	bg:setPosition(bg:getContentSize().width/2,bg:getContentSize().height/2+15)
	self:addChild(bg)
	self.bg = bg
	...
	deskNoBg:setPosition(size.width*0.5,5)
	bg:addChild(deskNoBg)

	local deskNo = FontConfig.createWithCharMap(self.deskInfo[self.index].iDeskNo,"desk/zhuohao-zitiao.png",13,17,"0")
