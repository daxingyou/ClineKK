...
local CreateRoomHistory = require("baijiale.CreateRoomHistory");
...
function CreateGoldRoomLayer:onClick(sender)
	...
	if tag == 1 then
		display.getRunningScene():addChild(require("baijiale.HelpLayer"):create());
	elseif tag == 2 then
		Hall:exitGame()
	end
end
...
function CreateGoldRoomLayer:onGameRecoredCallback(data)
	...
	local cf = {
		...
		{"RecordCount","INT"},
		{"pData","GameMsg.tagServerGameRecord[100]"},
	}
