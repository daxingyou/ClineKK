...
---------------------------------------------
	...
	local zdZi = FontConfig.createWithCharMap("","roomHistory/historyzitiao.png",16,23,"0");
	zdZi:setAnchorPoint(0,0.5);
	zdZi:setPosition(289,sumBg:getContentSize().height/2);
	sumBg:addChild(zdZi);
	self.zdZi = zdZi;

	local xdZi = FontConfig.createWithCharMap("","roomHistory/historyzitiao.png",16,23,"0");
	xdZi:setAnchorPoint(0,0.5);
	xdZi:setPosition(384,sumBg:getContentSize().height/2);
	sumBg:addChild(xdZi);
	self.xdZi = xdZi;
...
function CreateRoomHistory:RefreshAllLZ(msg)
	...
	self.zdZi:setString(zdCount);
	self.xdZi:setString(xdCount);
end
	...
	for k1,v1 in pairs(dataDeal) do
		for k2, v2 in pairs(v1) do
			...
			if v2.bBankerTwoPair then
				local dui = ccui.ImageView:create("roomHistory/historyzhuang.png");
				dui:setAnchorPoint(0,1);
				dui:setPosition(0,spSize.height);
				sp:addChild(dui);
			end

			if v2.bPlayerTwoPair then
				local dui = ccui.ImageView:create("roomHistory/historyxian.png");
				dui:setAnchorPoint(1,0);
				dui:setPosition(spSize.width,0);
				sp:addChild(dui);
			end
