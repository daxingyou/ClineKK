...
--新版大路排序
function LZLogic:GetNewBigWayList(msg,MaxHeight)
	local msg = clone(msg);
	...
	for i = 1,#msg do
		if i == 1 then
			...
			msg[i].flatCount = 0;
			if self:GetType(msg[i]) == LZType.flat then
				--强制更改
				msg[i].cbPlayerCount = 1;
				msg[i].cbBankerCount = 0;

				msg[i].flatCount = msg[i].flatCount + 1;
			end
			...
			--先判断是不是平 是平只需计数加1
			if self:GetType(msg[i]) == LZType.flat then
				local index = i - msg[i-1].flatCount-1;
				if index < 1 then
					index = 1;
				end

				local flatCount = msg[index].flatCount + 1;
				msg[i].flatCount = flatCount;
				msg[index].flatCount = flatCount;
				...
				if self:GetType(msg[index]) == self:GetType(msg[i]) then
					local point = msg[index].point;
...
function LZLogic:GetType(msg)
	if msg.cbPlayerCount>msg.cbBankerCount then
		return LZType.player;--闲
	elseif msg.cbPlayerCount<msg.cbBankerCount then
		return LZType.master;--庄
	elseif msg.cbPlayerCount==msg.cbBankerCount then
		return LZType.flat;--平
	end
end
...
function LZLogic:GetZLWayList(msg,MaxHeight)
	...
	for i = 1,#msg do
		...
		temp.bBankerTwoPair = msg[i].bBankerTwoPair;
		temp.bPlayerTwoPair = msg[i].bPlayerTwoPair;
		temp.lastFlag = false;
...
--新下3路的排序
function LZLogic:SortNewThreeOtherWay(msg)
	local msg = clone(msg);
		...
		if i == 1 then
			...
			if self:GetType(msg[i]) == LZType.flat then
				...
				if self:GetType(msg[index]) == self:GetType(msg[i]) then
					...
					msg[i].tpoint = cc.p(point.x,point.y+1);
					msg[i].count = 1;
					msg[i].flatCount = 0;
					msg[i].color = self:SetColor(msg[i]);
				end
...
function LZLogic:GetThreeWayList(msg,startPoint,MaxHeight)
	...
	local msg = clone(msg);
	local msg = self:SortNewThreeOtherWay(msg);
...
function LZLogic:GetFutureWay(msg,typeWay,startPoint)
	...
	local newData = {}
	if typeWay == 1 then--庄
		newData.cbPlayerCount = 1;
		newData.cbBankerCount = 2;
	else
		newData.cbPlayerCount = 2;
		newData.cbBankerCount = 1;
	end
