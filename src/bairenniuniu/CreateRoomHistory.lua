...
---------------------------------------------
	...
	bgPanel:setContentSize(530,221);
	bgPanel:setPosition(12,13);
	self:addChild(bgPanel);
	self.bgPanel = bgPanel;

	for i = 1,4 do
		local typeSp = ccui.ImageView:create("roomHistory/brnn_type"..i..".png");
		typeSp:setAnchorPoint(0,0);
		typeSp:setPosition(0,221/4*(4-i)+2);
		bgPanel:addChild(typeSp);
	end

	for i = 1,9 do
		local diSp = ccui.ImageView:create("roomHistory/historydi.png");
		diSp:setAnchorPoint(0,0);
		diSp:setPosition(40+53*(i-1),0);
		diSp:setName("di"..i);
		bgPanel:addChild(diSp,20-i);
	end
...
function CreateRoomHistory:UpdateLayer(data)
	...
	--过滤只剩10局
	if #dealPData>9 then
		for i = #dealPData - 9+1,#dealPData do
	...
	--先清除界面上的图片
	for i = 1,9 do
		local diSp = self.bgPanel:getChildByName("di"..i);
		if diSp then
			diSp:removeAllChildren();
		end
	end

	local m_jettonAreaTagTb ={1,2,4,8};
	...
	for k,v in pairs(data.pData) do
		...
		if diSp then
	    	...
	    	for j=1,4 do
	    		local path = "";
	    		if bit:_and(m_jettonAreaTagTb[j], v) ~= 0 then
	    			path = "roomHistory/brnn_gou.png";
	    		else
	    			path = "roomHistory/brnn_cha.png";
	    		end

	    		if path~="" then
	    			local dataSp = ccui.ImageView:create(path);
	    			dataSp:setAnchorPoint(0.5,0);
	    			dataSp:setPosition(size.width/2,size.height/4*(4-j)+7);
	...
	if gameStation > 0 then
		...
		if gameStation == GameMsg.GS_FREE then
			gameStatus = "等待中";
		elseif gameStation == GameMsg.GS_PLACE_JETTON then
			gameStatus = "下注中";
		elseif gameStation == GameMsg.GS_GAME_END then
			gameStatus = "开奖中";
		end
