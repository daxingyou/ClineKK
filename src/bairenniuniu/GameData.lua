...
--初始化用户列表
function GameData:initUserList( userList )
    luaDump(userList, "GameData:initUserList---------------------", 5)
    --以座椅号管理
    self.m_tableChairUserList = {}
    --以uid管理
    self.m_tableUidUserList = {}
    ...
    for k,v in pairs(userList) do
        self.m_tableChairUserList[v.bDeskStation + 1] = v;
...
--增加用户
function GameData:addUser( userItem )
    if nil == userItem then
        return;
    end

    self.m_tableChairUserList[userItem.bDeskStation + 1] = userItem;
    ...
    luaPrint("after add:" .. #self.m_tableList)
end

function GameData:updateUser( useritem )
    if nil == useritem then
        return
    end
...
function GameData:isUserInList( useritem )
    local user = nil
...
--移除用户
function GameData:removeUser( useritem )
    if nil == useritem then
        return
    end
    ...
    table.remove(self.m_tableChairUserList,useritem.bDeskStation + 1)
    table.remove(self.m_tableUidUserList,useritem.dwUserID)

    luaPrint("after remove:" .. #self.m_tableList)
end

function GameData:removeAllUser(  )
    --以座椅号管理
    self.m_tableChairUserList = {}
    --以uid管理
    self.m_tableUidUserList = {}
...
function GameData:getChairUserList(  )
    return self.m_tableChairUserList;
end

function GameData:getUidUserList(  )
    return self.m_tableUidUserList;
end

function GameData:getUserList( )
    return self.m_tableList
end

function GameData:sortList(  )
    --排序
...
--添加庄家申请用户
function GameData:addApplyUser( wchair,bRob )
    ...
    local info = GameMsg.getEmptyApplyInfo()
...
function GameData:getApplyUser( wchair )
    local user = nil
    for k,v in pairs(self.m_tableApplyList) do
        if v.m_userItem.bDeskStation == wchair then
            user = v
            break
        end
    end
...
--超级抢庄用户
--rob[抢庄用户] cur[当前用户]
function GameData:updateSupperRobBanker( rob,cur )
    if rob ~= cur then
...
--移除庄家申请用户
function GameData:removeApplyUser( wchair )
    local removeIdx = nil
    for k,v in pairs(self.m_tableApplyList) do
        if v.m_userItem.bDeskStation == wchair then
            removeIdx = k
...
--更新超级抢庄用户状态
function GameData:updateSupperRobState( wchair, state )
    local user = nil
    for k,v in pairs(self.m_tableApplyList) do
        if v.m_userItem.bDeskStation == wchair then
             --超级抢庄排最前
...
--获取申请庄家列表
function GameData:getApplyBankerUserList(  )
    return self.m_tableApplyList
end

---路单数据
function GameData:addGameRecord(rec)
...
--获取路单
function GameData:getGameRecord(  )
    return self.m_vecRecord
end

--清除路单
function GameData:clearRecord()
    self.m_vecRecord = {}
end
