...
function CreateGoldRoomLayer:initUI()
   ...
   --          if checkIphoneX() then
            ...
            elseif self.gameName == "slhb" then
                name = {"10bao","7bao"}
            elseif self.gameName == "game_lhdb" or self.gameName == "game_tgpd" or self.gameName == "game_shz" then
                name = {"pingmin","xiaozi","laoban","fuhao"}
            end
            local biaoti = ccui.ImageView:create("newExtend/vip/typeset/"..name[k]..".png")
		    biaoti:setPosition(size.width/2,size.height*0.6)
    ...
    local Label_name = FontConfig.createWithSystemFont("水\n浒\n传",26 ,cc.c3b(90, 44, 14));
...
function CreateGoldRoomLayer:onClick(sender)
	...
	if tag == 1 then
		-- display.getRunningScene():addChild(require("fqzs.HelpLayer"):create());
		self:addChild(require("baijiale.HelpLayer"):create());
	elseif tag == 2 then
		Hall:exitGame()
	end
end
