...
function CreateGoldRoomLayer:initUI()
	...
	local title = ccui.ImageView:create("createRoom/dezhoupuke.png")
...
function CreateGoldRoomLayer:createItem(tag)
	local difenNum = {nil,0.5,2,5,10}
	local ruchang = {nil,"50.00","200.00","500.00","1000.00"}
    ...
    if difenNum[tag] then
		-- local difen = FontConfig.createWithCharMap(difenNum[tag].."底分","game/createRoomduizhan/zitiao1.png",26,44,"+",{{"底分",":;<="}})
		-- difen:setPosition(size.width/2,size.height/2-130)
		-- btn:addChild(difen)

		local mangzhu = FontConfig.createWithSystemFont("小盲注: "..difenNum[tag])
		mangzhu:setAnchorPoint(1,0.5);
		mangzhu:setPosition(size.width/2-5,size.height/2-135)
		btn:addChild(mangzhu)
		mangzhu:setColor(cc.c3b(255,225,146));

		local mangzhu = FontConfig.createWithSystemFont("大盲注: "..(difenNum[tag]*2))
		mangzhu:setAnchorPoint(0,0.5);
		mangzhu:setPosition(size.width/2+5,size.height/2-135)
		btn:addChild(mangzhu)
		mangzhu:setColor(cc.c3b(255,225,146));
