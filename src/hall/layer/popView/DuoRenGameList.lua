local DuoRenGameList = class("DuoRenGameList", BaseWindow)

function DuoRenGameList:ctor()
...
function DuoRenGameList:initData()
	self.updateBtn = {}
	self.skBtn = {}
	self.listViewBtn = {}
end

function DuoRenGameList:bindEvent()
...
function DuoRenGameList:onEnterTransitionFinish()
...
function DuoRenGameList:onExit()
	for k,v in pairs(self.updateBtn) do
...
function DuoRenGameList:initUI()
	...
	local title = ccui.ImageView:create("hall/duorenTitle.png")
...
function DuoRenGameList:createGameList()
	...
	local path = {"kuaile30miao","longhudou","hongheidazhan","yaoyiyao","feiqinzoushou","bairenpinshi","benchibaoma"}
	local gameName = {"baijiale","longhudou","hongheidazhan","yaoyiyao","fqzs","bairenniuniu","benchibaoma"}
	local sGameName = {"game_30miao","game_lhd","game_hhdz","game_yyy","game_fqzs","game_brps","game_ppc"}
...
function DuoRenGameList:createGameItem(name,gameName,count)
	local layout = ccui.Layout:create()
	...
	local path = "hall/effect/duoren/"..name
	local ret = true
	...
	skeleton_animation:setPosition(size.width/2, size.height*0.47)	
	btn:addChild(skeleton_animation)

	table.insert(self.skBtn,btn)

	if ret == true then
		if name == "hongheidazhan" or name == "longhudou" then
			skeleton_animation:setPositionY(size.height*0.51)
		end
	else
		if name == "hongheidazhan" or name == "yaoyiyao" then
			skeleton_animation:setPositionY(size.height*0.48)
			skeleton_animation:setPositionX(size.width*0.5-10)
		elseif name == "feiqinzoushou" then
			skeleton_animation:setPositionY(size.height*0.46)
			skeleton_animation:setPositionX(size.width*0.5-8)
		elseif name == "bairenpinshi" then
			skeleton_animation:setPositionY(size.height*0.53)
			skeleton_animation:setPositionX(size.width*0.5-15)
		elseif name == "benchibaoma" then
			skeleton_animation:setPositionY(size.height*0.48)
			skeleton_animation:setPositionX(size.width*0.5+5)
...
--下载点击
function DuoRenGameList:onClickBtn(sender,isPlay)
...
function DuoRenGameList:onClick(sender)
...
--左右箭头处理
function DuoRenGameList:listViewSlider(tag,flag)
...
function DuoRenGameList:onScrollEvent(event)
	local ret = false
...
--设置下载回调
function DuoRenGameList:setDownCallback(func)
	self.callback = func
end

--点击下载时，同步其他的按钮
function DuoRenGameList:syncSameGameUpdate(data)
...
function DuoRenGameList:receiveExitCreateRoom(data)
	if data then
		data = data._usedata
	end
...
function DuoRenGameList:resetViewPos()
	local y = 130;
...
function DuoRenGameList:playEffect()
...
function DuoRenGameList:reDownload(data)
	local data = data._usedata
