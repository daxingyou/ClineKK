...
function CreateGoldRoomLayer:initUI()
	...
	if GamesInfoModule:findGameName(40010001) and GamesInfoModule:findGameName(40010005) then
		ret = true
		--模式按钮
	...
	if GamesInfoModule:findGameName(40010005) then
		if not ret then
			self.isRoomMode = 0
			globalUnit:setIsEnterRoomMode(self.isRoomMode)
		end
	...
	if GamesInfoModule:findGameName(40010001) then
		if not ret then
			self.isRoomMode = 1
			globalUnit:setIsEnterRoomMode(self.isRoomMode)
		end
...
function CreateGoldRoomLayer:createItem(tag,name)
	...
	if name ~= "tiyanchang" then
	 ...
	 --    btn:addChild(gold)
	    ...
	    local Roomid = {164,165,166,167}--ruchangNum[k] --最小金币限制
	    ...
	    local minRoomKey = RoomInfoModule:getRoomNeedGold(40010001,Roomid[tag])
...
function CreateGoldRoomLayer:onClick(sender)
	...
	if tag == 3 then
		display.getRunningScene():addChild(require("likuipiyu.popView.HelpLayer"):create());
	elseif tag == 2 then
		Hall:exitGame()
