...
local szGameName = {"lkpy_1","lkpy_5"}
...
function CreateGoldRoomLayer:initUI()
	...
	for k,v in pairs(self.gameList) do
        if "lkpy_1" == v.szGameName then
            x1 = 1
        end
        if "lkpy_5"== v.szGameName then
            x2 = 1
        end
    end
    ...
    local Label_name = FontConfig.createWithSystemFont("李\n逵\n劈\n鱼",26 ,cc.c3b(90, 44, 14));
...
function CreateGoldRoomLayer:createItem(parent,list,itype)
	...
	local GameNameID = VipRoomInfoModule:getuNameIDBysGameName("lkpy_1")
	local roomList_Roomid = VipRoomInfoModule:getRoomListBysGameName("lkpy_1")
	if itype == 1 then
		GameNameID = VipRoomInfoModule:getuNameIDBysGameName("lkpy_1")
	else
		GameNameID = VipRoomInfoModule:getuNameIDBysGameName("lkpy_5")
	end
...
function CreateGoldRoomLayer:refreshGameList(data)
	...
	if data == 1 then
		if GamesInfoModule:findGameName(40010001) or GamesInfoModule:findGameName(40010005) then
			self:initData()
...
function CreateGoldRoomLayer:enterGame(sender)
	...
	local roomid = {164,165,166,167}
	if self.isRoomMode == 1 then
		self.m_selectedNameID = 40010001;
	else
		self.m_selectedNameID = 40010005;
		roomid = {168,169,170,171}
	end
...
function CreateGoldRoomLayer:onClick(sender)
	...
	if tag == 3 then
		...
		self:addChild(require("likuipiyu.popView.HelpLayer"):create());
	elseif tag == 2 then
		Hall:exitGame()
