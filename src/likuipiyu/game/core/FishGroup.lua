...
function FishGroup:createFishGroup( parent, num, fish_type, path_type )
	...
	-- luaPrint("num "..num.." fish_type "..fish_type.." path_type "..path_type)
...
function FishGroup:createFishLineGroup( parent, num, fish_type, path_type )
	...
	local fishArray= {};
	-- luaPrint("fish Line group num : "..num.."  fish type:"..fish_type)
	for i=1,num do
		if(fishPf==nil)then
			...
			fishP._fishPath = {};
			interval = fishP._speedTime * fishP._radius*2/size.width+2;
			local _delay=interval*(i-1) + 1.5;
...
function FishGroup:createFishRandGroup( parent, num, fish_type, path_type )
	...
	local size = cc.Director:getInstance():getWinSize();
	local fishArray= {};
	-- luaPrint("fish Rand group num : "..num.."  fish type:"..fish_type)
	for i=1,num do
		if(fishPf==nil)then
			fishPf = FishManager:createSpecificFish(parent,fish_type,path_type);
			fishP = fishPf;
...
function FishGroup:createFishNormalGroup( parent, num, fish_type, path_type )
	...
	local size = cc.Director:getInstance():getWinSize();
	local fishArray= {};
	-- luaPrint("fish normal group num : "..num.."  fish type:"..fish_type)
	for i=1,num do
		if(fishPf==nil)then
			fishPf = FishManager:createSpecificFish(parent,fish_type,path_type);
			fishP = fishPf;
