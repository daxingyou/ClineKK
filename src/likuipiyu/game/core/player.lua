...
local FishManager = require("likuipiyu.game.core.FishManager"):getInstance();
...
function player:showLockFish(visible,lockFish)
    ...
    if lockFish ~= self.lockFish and visible and lockFish and not tolua.isnull(lockFish) then
        ...
        elseif v >= 28 and v <= 30 then--四喜
            ...
            str = "lkpy_fish_"..v.."_1.png";
        end
        display.loadSpriteFrames("likuipiyu/fishing/fish/lkpy_fish_test1.plist", "likuipiyu/fishing/fish/lkpy_fish_test1.png");
        display.loadSpriteFrames("likuipiyu/fishing/fish/lkpy_fish_test2.plist", "likuipiyu/fishing/fish/lkpy_fish_test2.png");
        display.loadSpriteFrames("likuipiyu/fishing/fish/lkpy_fish_test3.plist", "likuipiyu/fishing/fish/lkpy_fish_test3.png");
...
function player:setDefaultModel(container, database)
    ...
    local cannon = require("likuipiyu.game.core.cannon").new(container, database, seat, self, lv);
    ...
    self.labelValue = FontConfig.createWithCharMap(goldConvert(self.cur_zi_dan_cost), "game/zidannum.png", 20, 19, "+",{{"x","+"}})--FontConfig.createLabel(FontConfig.gFontConfig_28, self.cur_zi_dan_cost, cc.c3b(255,224,20));
    ...
    self.numberLayer = FontConfig.createWithCharMap(string.format("%.2f", FormatNumToString(goldConvert(self.score),1)), "game/moneyzitiao.png", 15, 19, "+")
    self.numberLayer:setAnchorPoint(0.5,0.5)
    self.numberLayer:setPosition(size.width/2-5,40)
    ...
    self.nickname:setPosition(size.width/2-5,17)
    ...
    if FLIP_FISH then
        if self.lSeatNo == 1 or self.lSeatNo == 4 then
            ...
            self.numberLayer:setPositionX(size.width/2+10);
            self.lv:setPositionX(self.lv:getPositionX()+60);
            self.nickname:setPositionX(size.width/2+10);
        end
    else
        if self.lSeatNo == 2 or self.lSeatNo == 3 then
            -- gold:setPositionX(gold:getPositionX()+245);
...
function player:createGuide()
    ...
    local guide = require("likuipiyu.game.core.guideLayer").new(GuideTypeCannonUnlock);
