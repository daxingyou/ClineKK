...
local TableLogic = require("hongheidazhan.TableLogic");
local GameRuleLayer = require("hongheidazhan.GameRuleLayer");
local LZLogic = require("hongheidazhan.LZLogic");
...
function TableLayer:ctor(uNameId, bDeskIndex, bAutoCreate, bMaster)
	...
	GameMsg.init();
	HhdzInfo:init();
	display.loadSpriteFrames("hongheidazhan/chip/chip.plist","hongheidazhan/chip/chip.png");
	...
	loadNewCsb(self,"hongheidazhan/GameScene",uiTable);
...
function TableLayer:bindEvent()
	...
	self:pushEventInfo(HhdzInfo,"ErrorCode",handler(self, self.DealErrorCode));
	self:pushEventInfo(HhdzInfo,"GameBegin",handler(self, self.DiceGameStartInfo));
	self:pushEventInfo(HhdzInfo,"NoteResult",handler(self, self.PutChipInfoGame));
	self:pushEventInfo(HhdzInfo,"OpenCard",handler(self, self.ResultMessageInfo));
	self:pushEventInfo(HhdzInfo,"ShangZhuang",handler(self, self.BankStateInfoGame));
	self:pushEventInfo(HhdzInfo,"WuZhuang",handler(self, self.DealNoBankWait));
	self:pushEventInfo(HhdzInfo,"ZhuangScore",handler(self, self.DealZhuangScore));
	self:pushEventInfo(HhdzInfo,"XuTOU",handler(self, self.DealXuTOU));
	
	self:pushEventInfo(HhdzInfo,"NoteBack",handler(self, self.DealNoteBack));
...
function TableLayer:initUI()
    ...
    --区块链bar
    self.m_qklBar = Bar:create("hongheidazhan",self);
    ...
    local deskNo = FontConfig.createWithCharMap(self.bDeskIndex+1,"number/honghei_zhuohao.png",24,32,"0")
	...
	for i = 1,6 do
		if self["Button_score"..i] then
			...
			self["Button_score"..i]:addClickEventListener(function(sender) 
				...
				if effectState then
					audio.playSound("hongheidazhan/sound/sound-jetton.mp3");
				end
...
function TableLayer:ClickOtherButtonCallBack(sender)
    ...
    if globalUnit.isShowZJ then
    	...
    	self.Image_otherBtnBg:loadTexture("honghei_setdi.png",UI_TEX_TYPE_PLIST);
    end
    self:InitSet();
	...
	if senderTag == 0 then--下拉
		...
		sender:loadTextures("honghei_xialaanniuup.png","honghei_xialaanniuup-on.png","honghei_xialaanniuup-on.png",UI_TEX_TYPE_PLIST);
	else--上拉
		sender:setTag(0);
		...
		sender:loadTextures("honghei_xialaanniu.png","honghei_xialaanniu-on.png","honghei_xialaanniu-on.png",UI_TEX_TYPE_PLIST);
	end
...
function TableLayer:ClickEffectCallBack(sender)
	...
	if effectState then--开着音效
		...
		--改变音效图片
		self.Button_effect:loadTextures("honghei_yinxiao-off.png","honghei_yinxiao-off-on.png","honghei_yinxiao-off-on.png",UI_TEX_TYPE_PLIST);
	else
		setEffectIsPlay(true);
		self.Button_effect:loadTextures("honghei_yinxiao.png","honghei_yinxiao-on.png","honghei_yinxiao-on.png",UI_TEX_TYPE_PLIST);	
	end
...
function TableLayer:ClickMusicCallBack(sender)
	...
	if musicState then--开着音效
		...
		self.Button_music:loadTextures("honghei_yinyue-off.png","honghei_yinyue-off-on.png","honghei_yinyue-off-on.png",UI_TEX_TYPE_PLIST);
	else
		setMusicIsPlay(true);
		self.Button_music:loadTextures("honghei_yinyue.png","honghei_yinyue-on.png","honghei_yinyue-on.png",UI_TEX_TYPE_PLIST);	
	end
            ...
            if self.gameBet and clockTag == 3 then--如果是下注状态最后几秒显示动画
	        	...
	        	self.Node_startAnimate:runAction(cc.Sequence:create(
					cc.DelayTime:create(0),cc.CallFunc:create(function()
        				...
        				if effectState then
        					audio.playSound("hongheidazhan/sound/sound-clock-count.mp3");
        				end
		        	end),
...
function TableLayer:InitSet()
	...
	display.loadSpriteFrames("hongheidazhan/hongheidazhan.plist","hongheidazhan/hongheidazhan.png");
	--保险箱
	self.Button_insurance:loadTextures("honghei_baoxianxiangsp.png","honghei_baoxianxiangsp-on.png","honghei_baoxianxiangsp-on.png",UI_TEX_TYPE_PLIST);
	--规则
	self.Button_rule:loadTextures("honghei_guize.png","honghei_guize-on.png","honghei_guize-on.png",UI_TEX_TYPE_PLIST);
	--音效
	local effectState = getEffectIsPlay();
	if effectState then--开着音效
		self.Button_effect:loadTextures("honghei_yinxiao.png","honghei_yinxiao-on.png","honghei_yinxiao-on.png",UI_TEX_TYPE_PLIST);	
...
function TableLayer:InitMusic()
	local musicState = getMusicIsPlay();
	if musicState then
		audio.playMusic("hongheidazhan/sound/sound-bg.mp3", true)
	else
		audio.stopMusic();
	end
end
...
function TableLayer:getRandomPosByType(type1)--type 1,2,3
	...
	if type1 == 3 then
		...
		randomPos.y = randomPos.y + math.random(0,55)-45;
	else
		randomPos.x = randomPos.x + math.random(0,200)-100;
		randomPos.y = randomPos.y + math.random(0,55)-50;
	end
	return randomPos;
end
...
function TableLayer:playZitiEffect(iType)
	...
	if iType == 3 then
		...
		-- 	colorLayer:removeFromParent();
		-- end
		local ziSpine = createSkeletonAnimation("hongheidazhan1","hongheidazhan/cartoon/hongheidazhan.json","hongheidazhan/cartoon/hongheidazhan.atlas");
		if ziSpine then
			local winSize = cc.Director:getInstance():getWinSize()
			ziSpine:setPosition(winSize.width/2,winSize.height/2);
			ziSpine:setAnimation(1,"hongheidazhan", false);
			ziSpine:setName("longhudouziSpine");
			self:addChild(ziSpine,11);

			local effectState = getEffectIsPlay();
			if effectState then
				audio.playSound("hongheidazhan/sound/sound-battle.mp3");
			end
		end
	end
	self.Node_startAnimate:runAction(cc.Sequence:create(cc.DelayTime:create(time),cc.CallFunc:create(function ()--展示闪烁再显示结算和动画
		...
		if iType == 3 then
			audio.playSound("hongheidazhan/sound/sound-start-wager.mp3");--开始下注
		end
...
function TableLayer:SetButtonVillageState(buttonState)
	...
	if buttonState == villageState.zUserStatus_Null then--空闲状态
		self.Button_village:setTag(0);
		self.Button_village:loadTextures("honghei_shangzhuang.png","honghei_shangzhuang-on.png","honghei_shangzhuang-on.png",UI_TEX_TYPE_PLIST);
	elseif buttonState == villageState.zStatus_IsZhuang then--坐庄状态
		self.Button_village:setTag(2);
		self.Button_village:loadTextures("honghei_xiazhuang.png","honghei_xiazhuang-on.png","honghei_xiazhuang-on.png",UI_TEX_TYPE_PLIST);
	elseif buttonState == villageState.zUserStatus_InList then--申请上庄状态
		self.Button_village:setTag(1);
		self.Button_village:loadTextures("honghei_qxsz.png","honghei_qxsz-on.png","honghei_qxsz-on.png",UI_TEX_TYPE_PLIST);
	elseif buttonState == villageState.zStatus_XiaZhuang then--申请下庄状态
		self.Button_village:setTag(3);
		self.Button_village:loadTextures("honghei_qxxz.png","honghei_qxxz-on.png","honghei_qxxz-on.png",UI_TEX_TYPE_PLIST);
	end
...
function TableLayer:BankStateInfoGame(message)
	...
	elseif data.success == 4 then--换庄
		...
		local effectState = getEffectIsPlay();
		if effectState then
			audio.playSound("hongheidazhan/sound/zhuang-change.mp3");
		end
...
function TableLayer:DealPutChipMsg(msg)
	...
	for i,data in ipairs(msg) do
		...
		local effectState = getEffectIsPlay();
	    if effectState then
	    	audio.playSound("hongheidazhan/sound/sound-betlow.mp3");
		end
	end
...
function TableLayer:DiceGameStartInfo(message)
	...
	self.Image_gameState:loadTexture("honghei_xiazhuzhong.png",UI_TEX_TYPE_PLIST);
	--重置下注时间
...
function TableLayer:ResultMessageInfo(message)
	...
	if self.pourChip == false and self.bankStation~=self.tableLogic:getMySeatNo() then
		...
		--audio.playSound("hongheidazhan/sound/sound-result-nobet.mp3")
	end

	self:playZitiEffect(4);
	audio.playSound("hongheidazhan/sound/sound-end-wager.mp3");
	
	self.Image_gameState:setVisible(true);
	display.loadSpriteFrames("hongheidazhan/hongheidazhan.plist","hongheidazhan/hongheidazhan.png");
	self.Image_gameState:loadTexture("honghei_kaijiangzhong.png",UI_TEX_TYPE_PLIST);
	...
	self:createCard(data.iRBCard,data.iCardShape,data.iWinner);
	...
	self.Image_bg:runAction(cc.Sequence:create(cc.DelayTime:create(9),cc.CallFunc:create(function ()--展示闪烁再显示结算和动画
		if self.gameStart == false then
			return;
		end
		...
		if data.iWinner ~= 2 then
			local bipaitexiao = createSkeletonAnimation("honghei","hongheidazhan/cartoon/honghei.json","hongheidazhan/cartoon/honghei.atlas");
			if bipaitexiao then
				bipaitexiao:setName("bipaitexiao");
				self.Node_bipaicartoon:addChild(bipaitexiao,10);

				if data.iWinner == 0 then--红
					bipaitexiao:setAnimation(1,"hongying", false);
					bipaitexiao:setPosition(chipRandomPos[1]);
				elseif data.iWinner == 1 then--黑
					bipaitexiao:setAnimation(1,"heiying", false);
					bipaitexiao:setPosition(chipRandomPos[2]);
				end

			end
				if data.iWinner == 0 then--红
					audio.playSound("hongheidazhan/sound/hongwin.mp3");
				elseif data.iWinner == 1 then--黑
					audio.playSound("hongheidazhan/sound/heiwin.mp3");
				end
		end
		...
		if isSound then
			audio.playSound("hongheidazhan/sound/sound-get-gold.mp3")
		end	

	end)));

	self.Image_bg:runAction(cc.Sequence:create(cc.DelayTime:create(9.8),cc.CallFunc:create(function ()--等待输钱飞到庄家后执行
		if self.gameStart == false then
			return;
		end
		local isSound = false;
	...
	self.Image_bg:runAction(cc.Sequence:create(cc.DelayTime:create(10.6),cc.CallFunc:create(function ()--等待赢的钱飞向赢的区域后执行--将钱飞向玩家
		--如果自己选的区域赢钱则创建对象飞向自己
		if self.gameStart == false then
			return;
		end
		...
		--人物动画
		--if data.iWinner == 0 then
			--self:showRenwuAnimation(1,3);
			--self:showRenwuAnimation(2,2);
		--else
			--self:showRenwuAnimation(1,2);
			--self:showRenwuAnimation(2,3);
		--end
		...
		if data.iUserScore>0 then
			if data.iUserReturnTotle >= 0 then
				self.Text_playerChange:setProperty("","hongheidazhan/number/zi_ying.png",26,50,'+');
				self:ScoreToMoney(self.Text_playerChange,data.iUserScore,"+");
				self:MoneyActionShow(self.Text_playerChange);

				if data.iCardShape[data.iWinner+1] > 2 then
					audio.playSound("hongheidazhan/sound/bigMoney.mp3");--大牌音效
				else
					audio.playSound("hongheidazhan/sound/win.mp3");--赢
				end
			end
			
		elseif data.iUserScore<0 then
			self.Text_playerChange:setProperty("","hongheidazhan/number/zi_shu.png",26,50,'+');
			self:ScoreToMoney(self.Text_playerChange,data.iUserScore);
			self:MoneyActionShow(self.Text_playerChange);
			audio.playSound("hongheidazhan/sound/lose.mp3");--输
		end

		--显示庄家获得金币显示
		if data.iNTScore > 0  then
			self.Text_bankMoneyChange:setProperty("","hongheidazhan/number/zi_ying.png",26,50,'+');
		...
		elseif data.iNTScore<0 then
			self.Text_bankMoneyChange:setProperty("","hongheidazhan/number/zi_shu.png",26,50,'+');
			self:ScoreToMoney(self.Text_bankMoneyChange,data.iNTScore);
			self:MoneyActionShow(self.Text_bankMoneyChange);
		end
		...
		if data.iOtherScore > 0 then
			self.Text_otherPlayerChange:setProperty("","hongheidazhan/number/zi_ying.png",26,50,'+');
		...
		elseif data.iOtherScore < 0 then
			self.Text_otherPlayerChange:setProperty("","hongheidazhan/number/zi_shu.png",26,50,'+');
			self:ScoreToMoney(self.Text_otherPlayerChange,data.iOtherScore);
			self:MoneyActionShow(self.Text_otherPlayerChange);
		end
		--计算玩家和庄家分数的增减
...
function TableLayer:ReConnectGame(msg,gameState)
	...
	self.Button_throw:setEnabled(false);--游戏开始续投置灰
	self.GameStatus = tonumber(msg.bGameStation);
...
--创建牌先移除原先的，在展示发牌动画
function TableLayer:createCard(cardData,cardShape,iWinner)
	display.loadSpriteFrames("hongheidazhan/card.plist","hongheidazhan/card.png");
	display.loadSpriteFrames("hongheidazhan/hongheidazhan.plist","hongheidazhan/hongheidazhan.png");
	local iCardShape = clone(cardShape)
	if iCardShape[1] == 8 then
		iCardShape[1] = 3
	end
	if iCardShape[2] == 8 then
		iCardShape[2] = 3
	end
	--创建结算的底
	local bipaiBg = cc.Sprite:createWithSpriteFrameName("honghei_bipaidi.png");
	bipaiBg:setAnchorPoint(0.5,1);
	bipaiBg:setPosition(640,720);
	self.Node_bipaicartoon:addChild(bipaiBg,10);
	...
	for i=1,6 do
		...
		local time = 0;
		if i <= 3 then
			card:setPosition(255+(i-1)*100,610);
		else
			card:setPosition(725+(i-3)*100,610);
	...
	for index,card in pairs(self.cardTable) do
		...
		local seq1 = cc.Sequence:create(cc.DelayTime:create(time),cc.OrbitCamera:create(0.2,1,0,0,-90,0,0),cc.Hide:create());
		local call = cc.CallFunc:create(function ()
			...
			local path = cardData[math.floor((index-1)/3)+1][y];
			audio.playSound("hongheidazhan/sound/sound-fanpai.mp3");--翻牌
		...
		local call2 = cc.CallFunc:create(function ()
			if index == 3 then
				local cardTyeBg = cc.Sprite:createWithSpriteFrameName("honghei_typexianshi.png");
				cardTyeBg:setPosition(355,520);
				self.Node_bipaicartoon:addChild(cardTyeBg,10);

				local size = cardTyeBg:getContentSize();
				local cardTye = cc.Sprite:createWithSpriteFrameName("honghei_type"..(tonumber(iCardShape[1]))..".png");
				cardTye:setPosition(size.width/2,size.height/2);
				cardTyeBg:addChild(cardTye);

				audio.playSound("hongheidazhan/sound/sound-cardtype-"..(tonumber(iCardShape[1])-1)..".mp3");--翻牌
			elseif index == 6 then
				local cardTyeBg = cc.Sprite:createWithSpriteFrameName("honghei_typexianshi.png");
				cardTyeBg:setPosition(925,520);
				self.Node_bipaicartoon:addChild(cardTyeBg,10);

				local size = cardTyeBg:getContentSize();
				local cardTye = cc.Sprite:createWithSpriteFrameName("honghei_type"..(tonumber(iCardShape[2]))..".png");
				cardTye:setPosition(size.width/2,size.height/2);
				cardTyeBg:addChild(cardTye);

				audio.playSound("hongheidazhan/sound/sound-cardtype-"..(tonumber(iCardShape[2])-1)..".mp3");--翻牌

				local shengliBg = cc.Sprite:createWithSpriteFrameName("honghei_bipaishengli.png");
				shengliBg:setAnchorPoint(0.5,1);
				if iWinner == 0 then
					shengliBg:setPosition(357,717);
				else
					shengliBg:setPosition(925,717);
				end
				self.Node_bipaicartoon:addChild(shengliBg,10);
				shengliBg:setOpacity(0);
				local fadeInAction = cc.FadeIn:create(0.3);
				local fadeOutAction = cc.FadeOut:create(0.3);

				shengliBg:runAction(cc.Sequence:create(fadeInAction,cc.Repeat:create(cc.Sequence:create(fadeOutAction,fadeInAction),2)));
			end
		end);
...
--开奖状态中的翻牌动画
function TableLayer:showCardOrbit(cardData,cardShape)
	display.loadSpriteFrames("hongheidazhan/card.plist","hongheidazhan/card.png");
	local cardData1 = cardData[2];
	local cardData2 = cardData[1];
	local historyTable = {"honghei_danzhang_2.png","honghei_duizi_1.png","honghei_shunzi_2.png","honghei_jinhua_2.png","honghei_shunjin_2.png","honghei_baozi_2.png"}
	local iCardShape = clone(cardShape)
	if iCardShape[2] == 8 then
		iCardShape[2] = 3
	end
	...
	local heicardType = historyTable[tonumber(iCardShape[2])-1];
	local hongcardType = historyTable[tonumber(iCardShape[1])-1];
	for index,card in pairs(self.cardTable) do
		local time = 0.2*index;
		if index >= 4 then
			time = time + 0.6
		end
		local seq1 = cc.Sequence:create(cc.DelayTime:create(time),cc.OrbitCamera:create(0.2,1,0,0,-90,0,0),cc.Hide:create());
		local call = cc.CallFunc:create(function ()
			local path = 0;
			if index <= 3 then
				path = cardData1[index];
			else
				path = cardData2[index-3];
			end
			audio.playSound("hongheidazhan/sound/sound-fanpai.mp3");--翻牌
			card:setSpriteFrame(cc.SpriteFrameCache:getInstance():getSpriteFrame(string.format("dzpk_0x%02X.png",path)));
		end);
		...
		local seq4 = cc.Sequence:create(cc.DelayTime:create(0.2));
		local call2 = cc.CallFunc:create(function ()
			self.Image_heipai:setVisible(true);
			self.Image_heipai:loadTexture(heicardType,UI_TEX_TYPE_PLIST);
			if index == 3 then
				audio.playSound("hongheidazhan/sound/sound-cardtype-"..(tonumber(iCardShape[2])-1)..".mp3");--翻牌
			end
		end);
		...
		if index > 3 then
			call2 = cc.CallFunc:create(function ()
				self.Image_hongpai:setVisible(true);
				self.Image_hongpai:loadTexture(hongcardType,UI_TEX_TYPE_PLIST);
				if index == 6 then
					audio.playSound("hongheidazhan/sound/sound-cardtype-"..(tonumber(iCardShape[1])-1)..".mp3");--翻牌
				end
			end);
		...
		card:runAction(cc.Sequence:create(seq1,call,seq2,seq4,call2));
	end

end

--获取有效的历史实录
function TableLayer:getEffectiveData(num,record)
	local realRecordTable = {};
	--luaDump(self.recordData)
	local temp = record or self.recordData;
...
function TableLayer:RefreshAllLZ(msg,isNeedFlush)
	...
	for k,v in pairs(msg) do
		...
		-- dump(v.iCardShape,"v.iCardShape")
		if v.iCardShape > 2 and v.iCardShape ~= 8 then--#define SH_DOUBLE_XIAOYU10      8        //小于10的对子
			pCount = pCount+1;
		end
	end
...
function TableLayer:RefreshZhuLZ(msg,isNeedFlush)
	...
	for k1,v1 in pairs(dataDeal) do
		for k2, v2 in pairs(v1) do
			--2龙 1虎 0平
			local str = "honghei_lz2_"..v2.type..".png";
...
function TableLayer:RefreshDaLZ(msg,isNeedFlush)
	...
	local dataDeal = LZLogic:GetBigWayList(msg,lzHeight);
	dataDeal = LZLogic:GetMaxLength(dataDeal,lzWidth);
	...
	for k1,v1 in pairs(dataDeal) do
		for k2, v2 in pairs(v1) do
			--2龙 1虎 0平
			local str = "honghei_lz"..v2.color..".png";
			...
			if v2.iCardShape>2 then
				local size = sp:getContentSize();
				local specialSp = cc.Sprite:createWithSpriteFrameName("honghei_landian.png");
				specialSp:setAnchorPoint(1,1);
				specialSp:setPosition(size.width,size.height);
				sp:addChild(specialSp);
			end
...
function TableLayer:RefreshYanLZ(msg,isNeedFlush)
	...
	for k1,v1 in pairs(dataDeal) do
		for k2, v2 in pairs(v1) do
			--2龙 1虎 0平
			local str = "honghei_lz3_"..v2.color..".png";
...
function TableLayer:RefreshXiaoLZ(msg,isNeedFlush)
	...
	for k1,v1 in pairs(dataDeal) do
		for k2, v2 in pairs(v1) do
			--2龙 1虎 0平
			local str = "honghei_lz4_"..v2.color..".png";
...
function TableLayer:RefreshRyLZ(msg,isNeedFlush)
	...
	for k1,v1 in pairs(dataDeal) do
		for k2, v2 in pairs(v1) do
			--2龙 1虎 0平
			local str = "honghei_lz5_"..v2.color..".png";
...
function TableLayer:RefreshLwlLZ(msg)
	...
	if lType then
		local str = "honghei_lz"..lType..".png";
		local sp = cc.Sprite:createWithSpriteFrameName(str);
		sp:setAnchorPoint(0,0);
	...
	if lType then
		local str = "honghei_lz6_"..lType..".png";
		local sp = cc.Sprite:createWithSpriteFrameName(str);
		sp:setAnchorPoint(0,0);
	...
	if lType then
		local str = "honghei_lz7_"..lType..".png";
		local sp = cc.Sprite:createWithSpriteFrameName(str);
		sp:setAnchorPoint(0,0);
...
--刷新幸运一击
function TableLayer:RefreshTypeLz(msg)
	self.Panel_gameType:removeAllChildren();

	local MaxCount = 11;

	local dealData = clone(msg);
	local tempData = clone(msg)
	if #msg > 11 then
		dealData = {};
		for i = 1,11 do
			table.insert(dealData,tempData[#tempData-11+i]);
		end
	end

	-- luaDump(dealData,"刷新幸运一击");
	for k,v in pairs(dealData) do
		if v.iCardShape == 8 then --小于10的对子
			v.iCardShape = 3
		end
		local typeSp = cc.Sprite:createWithSpriteFrameName("honghei_lztype"..v.iCardShape..".png");
		typeSp:setAnchorPoint(0,0);
		typeSp:setPosition((3.9+typeSp:getContentSize().width)*(k-1),0);
		self.Panel_gameType:addChild(typeSp);

		if k == #dealData then
			local size = typeSp:getContentSize();
			local newSp = cc.Sprite:createWithSpriteFrameName("honghei_xin.png");
			newSp:setAnchorPoint(1,1);
			newSp:setPosition(size.width+5,size.height+3);
			typeSp:addChild(newSp);
		end
	end
end
...
function TableLayer:ShowMoneyChange(msg)
	...
	for i=1,6 do
		if self["Image_player"..i]:getTag()~=-1 and self["Image_player"..i].userId == msg.dwUserID then
			self["Image_player"..i]:runAction(cc.Sequence:create(cc.DelayTime:create(10.6),cc.CallFunc:create(function()
				if self.gameStart == false then
					return;
				end
				...
				if msg.dwMoney > 0 then
					AtlasLabel_score:setProperty("","hongheidazhan/number/zi_ying.png",26,50,'+');
					self:ScoreToMoney(AtlasLabel_score,msg.dwMoney,"+");
					self:MoneyActionShow(AtlasLabel_score);
				elseif msg.dwMoney < 0 then
					AtlasLabel_score:setProperty("","hongheidazhan/number/zi_shu.png",26,50,'+');
					self:ScoreToMoney(AtlasLabel_score,msg.dwMoney);
					self:MoneyActionShow(AtlasLabel_score);
				end
