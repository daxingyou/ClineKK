...
function LhdInfo:init()
	self:initData();

	self:registerCmdNotify();
end

function LhdInfo:clear()
	self:clearAllRegisterCmdNotify()
end

function LhdInfo:initData()
	self.cmdList = {
		--//S-C
...
function LhdInfo:registerCmdNotify()
	self:clearAllRegisterCmdNotify()
...
--注销本表注册的所有事件
function LhdInfo:clearAllRegisterCmdNotify()
...
function LhdInfo:onReceiveCmdResponse(mainID, subID, object)
