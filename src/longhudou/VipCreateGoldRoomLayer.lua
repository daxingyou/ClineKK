...
function CreateGoldRoomLayer:initUI()
   ...
   --          if checkIphoneX() then
    ...
    local Label_name = FontConfig.createWithSystemFont("龙\n虎\n斗",26 ,cc.c3b(90, 44, 14));
...
function CreateGoldRoomLayer:onClick(sender)
	...
	if tag == 1 then
		-- display.getRunningScene():addChild(require("longhudou.GameRuleLayer"):create());
		self:addChild(require("longhudou.GameRuleLayer"):create());
	elseif tag == 2 then
		Hall:exitGame()
	end
end
