...
function TableLogic:I_R_M_GamePoint(object, objectSize)
	if self._callback.gameBet then
		self._callback:UpdateUserList(true);

		self._callback:ShowMoneyChange(object.data);
	end
	if self._callback.bankStation ~= self:getMySeatNo() then
		self._callback:ChipJundge(false);
	end
end
...
function TableLogic:dealGameStationResp(object, objectSize)
	...
	luaPrint("游戏状态",self._gameStatus);
	luaDump(msg,"场景消息");
	self._callback:ReConnectGame(msg,self._gameStatus);
end
...
--发送庄改变申请
function TableLogic:sendBankerState(bankState,cancel)--bankState是否上下庄
	luaPrint("发送庄改变申请",bankState,cancel,self._deskNo,self._mySeatNo)
	msg = {};
	msg.bCancel = cancel;
	msg.shang = bankState;
	msg.bIsRobot = false;

	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY, GameMsg.SUB_C_SHANG_ZHUANG, msg, GameMsg.Cr_Packet_SXZhuang)
end
--发送下注信息
function TableLogic:sendPutChipInfo(chipArea,chipMoney,chipType)--chipType筹码类型 chipArea筹码区域
	luaPrint("发送下注信息",chipArea,chipMoney,chipType)
	msg = {};
	msg.type = chipArea;
	msg.money = chipMoney;
	msg.moneytype = chipType;
	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY, GameMsg.SUB_C_NOTE, msg, GameMsg.Cr_Packet_XiaZhu)
end

--发送续投
function TableLogic:sendXuTouMsg(data)
	msg = {};
	msg.money = data;
	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY, GameMsg.SUB_C_NOTE_EX, msg, GameMsg.Cr_Packet_XiaZhuEx)
end
