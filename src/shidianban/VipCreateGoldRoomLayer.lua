...
function CreateGoldRoomLayer:initUI()
	...
	local difenNum = {"1","5","10","20"}
	local ruchangNum = {"50.00","250.00","500.00","1000.00"}
   ...
   --          if checkIphoneX() then
      ...
      --       local dikuang = ccui.ImageView:create("newExtend/vip/typeset/dikuang.png")
		    ...
		    ruchang:setPosition(size.width*0.65,size.height*0.38)
            ...
            difen:setPosition(size.width*0.35,size.height*0.38)
...
function CreateGoldRoomLayer:onClick(sender)
	...
	if tag == 1 then
    	...
    	self:addChild(PopUpLayer:create(),gameZorder);
	elseif tag == 2 then
		Hall:exitGame()
