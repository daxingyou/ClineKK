...
function TableLogic:dealGameStationResp(object, objectSize)
	...
	elseif self._gameStatus == GameMsg.GS_TH_QIANG then--//抢庄
    	local msg = convertToLua(object,GameMsg.CMD_S_StatusQiang);
    	luaDump(msg,"场景消息CMD_S_StatusQiang");
    	self:gameStationQiang(msg);--空闲状态
	elseif self._gameStatus == GameMsg.GS_PLAYING then
		local msg = convertToLua(object,GameMsg.CMD_S_StatusScore);
		luaDump(msg,"场景消息CMD_S_StatusScore");
		self:gameStationScore(msg);----下注状态
	elseif self._gameStatus == GameMsg.GS_TH_PLAY then
		local msg = convertToLua(object,GameMsg.CMD_S_StatusPlay);
		luaDump(msg,"场景消息CMD_S_StatusPlay");
		self._callback:gameStationPlay(msg);----游戏状态
	elseif self._gameStatus == GameMsg.GS_TK_BEGIN then
		self._callback:gameStationStart();----开始状态
	end
...
function TableLogic:gameStationScore(msg)
	self._callback:gameStationScore(msg)
end

function TableLogic:gameStationPlay(msg)
	self._callback:gameStationPlay(msg)
end
...
function TableLogic:I_R_M_GamePoint(msg)
	...
	if self._callback.gameStart == false then
		self._callback:UpdateUserMoney();
	end
end
...
function TableLogic:dealGameBuymoney(msg)
	if self._callback.gameStart then
...
--下注
function TableLogic:SendPutScore(cbCallScore)
	local msg = {};
	msg.lScore = math.floor(cbCallScore);
	luaPrint("下注-------------------",cbCallScore)
	luaDump(msg)
	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY,GameMsg.SUB_C_SCORE,msg,GameMsg.CMD_C_Score);
end
--叫牌
function TableLogic:SendGiveCard()
	luaPrint("叫牌-------------------")
	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY,GameMsg.SUB_C_GIVE_CARD);
end
--停牌
function TableLogic:SendGiveUp()
	luaPrint("停牌-------------------")
	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY,GameMsg.SUB_C_GIVE_UP);
end
--翻倍
function TableLogic:SendAddScore()
	luaPrint("翻倍-------------------")
	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY,GameMsg.SUB_C_ADD_SCORE);
end
--继续游戏
function TableLogic:SendGameContinue()
	luaPrint("继续游戏-------------------")
	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY,GameMsg.SUB_C_Continue);
end

--抢庄
function TableLogic:SendPlayerQiang(isQiang)
	luaPrint("抢庄-------------------")
	local msg = {};
	msg.bQiang = isQiang;
...
--放弃庄
function TableLogic:SendGiveUpBank(isGiveUp)
	luaPrint("放弃庄-------------------")
	local msg = {};
	msg.bGiveUp = isGiveUp;
