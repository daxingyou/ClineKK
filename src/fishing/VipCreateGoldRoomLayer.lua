...
function CreateGoldRoomLayer:initUI()
    ...
    local Label_name = FontConfig.createWithSystemFont("捕\n鱼\n达\n人",26 ,cc.c3b(90, 44, 14));
...
function CreateGoldRoomLayer:createItem(parent,list,itype)
	...
	local GameNameID = VipRoomInfoModule:getuNameIDBysGameName("lkpy_0")
	local roomList_Roomid = VipRoomInfoModule:getRoomListBysGameName("lkpy_0")
	if itype == 1 then
		GameNameID = VipRoomInfoModule:getuNameIDBysGameName("lkpy_0")
	else
		GameNameID = VipRoomInfoModule:getuNameIDBysGameName("lkpy_4")
	end
...
function CreateGoldRoomLayer:onClick(sender)
	...
	if tag == 3 then
		...
		self:addChild(require("fishing.popView.HelpLayer"):create());
	elseif tag == 2 then
		Hall:exitGame()
