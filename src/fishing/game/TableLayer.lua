...
local TableLogic = require("fishing.game.TableLogic");
local ResultLayer = require("fishing.game.ResultLayer");
local HelpLayer = require("fishing.popView.HelpLayer");
--捕鱼
FishManager = require("fishing.game.core.FishManager"):getInstance();
local FishGroup = require("fishing.game.core.FishGroup"):getInstance();
local FishMatrix = require("fishing.game.core.FishMatrix"):getInstance();
...
function TableLayer:ctor(uNameId, bDeskIndex, bAutoCreate, bMaster)
	...
	display.loadSpriteFrames("fishing/fishing/game/gamefish.plist", "fishing/fishing/game/gamefish.png")
...
function TableLayer:onEnter()
	...
	-- 	player = require("fishing.game.core.matchPlayer");
	-- else
		player = require("fishing.game.core.player");
	-- end
...
function TableLayer:initGameNormalBtn()
	...
	if self.Button_task then
		...
		--任务换成自动
		display.loadSpriteFrames("fishing/fishing/game/gamefish.plist", "fishing/fishing/game/gamefish.png")
		-- self.Button_task:loadTextures("zidong.png","zidong-on.png","zidong.png",UI_TEX_TYPE_PLIST);
		try({
			function() self.Button_task:loadTextures("zidong.png","zidong-on.png","zidong.png",UI_TEX_TYPE_PLIST) end,
			catch = function(error)
				display.removeSpriteFrames("fishing/fishing/game/gamefish.plist", "fishing/fishing/game/gamefish.png")
				display.loadSpriteFrames("fishing/fishing/game/gamefish.plist", "fishing/fishing/game/gamefish.png")
				self.Button_task:loadTextures("zidong.png","zidong-on.png","zidong.png",UI_TEX_TYPE_PLIST)
			end
		})
	...
	if self.Button_show then
		display.loadSpriteFrames("fishing/fishing/game/gamefish.plist", "fishing/fishing/game/gamefish.png")
		-- self.Button_show:loadTextures("yincang.png","yincang-on.png","yincang.png",UI_TEX_TYPE_PLIST);
		try({
			function() self.Button_show:loadTextures("yincang.png","yincang-on.png","yincang.png",UI_TEX_TYPE_PLIST) end,
			catch = function(error)
				display.removeSpriteFrames("fishing/fishing/game/gamefish.plist", "fishing/fishing/game/gamefish.png")
				display.loadSpriteFrames("fishing/fishing/game/gamefish.plist", "fishing/fishing/game/gamefish.png")
				self.Button_show:loadTextures("yincang.png","yincang-on.png","yincang.png",UI_TEX_TYPE_PLIST)
...
function TableLayer:onClickShowBtn(sender)
	...
	if sender:getTag() == 1 then--显示
		...
		sender:setTag(1);
		-- sender:setRotation(45);
		self.Button_show:loadTextures("show.png","show-on.png","show.png",UI_TEX_TYPE_PLIST);
	end
end
...
function TableLayer:onClickDistanceCallBack()
	luaPrint("距离按钮回调函数")
	local ScanLayer =  require("fishing.game.ScanLayer");
...
function TableLayer:onClickDistance(sender)
	if self.isShowPlayInfo then
		...
		if userInfo ~= nil then
			local DistanceLayer =  require("fishing.game.DistanceLayer");
...
function TableLayer:addPlayer(userInfo)
	...
	if userInfo.dwUserID == PlatformLogic.loginResult.dwUserID then
		...
		if not self:getChildByName("robot") then
			local layer = require("fishing.game.RobotManager").new()
...
-- local fish = self:addFish(41,math.random(1,6000),10);
	...
	-- performWithDelay(self, function() FishMatrix:createFishMatrix(self, 1,{}); end,10);
...
function TableLayer:onCatchBomFish(msg)
	...
	if tarFish and tarFish._fishType == 24 and not self.Sprite_bg:getChildByTag(10001) then
		local layer = require("fishing.game.core.quanPingArmatureLayer").new(pos)
		layer:setTag(10001)
...
function TableLayer:catchFishWithNet(bulletNode)
	if bulletNode.myFire then
		...
		-- end
	elseif bulletNode:isRobot() and self.playerNodeInfo[bulletNode.charID-1] and self.playerNodeInfo[bulletNode.charID-1]:getIsAlive()==true then
		-- Hall.showTips("机器人捕鱼了charID ="..charID.."  ,"..self.playerNodeInfo[bulletNode.charID].cur_zi_dan_cost);
		if self.tableLogic then
			self.tableLogic:sendCatchFish(bulletNode.paoLevel, bulletNode.id, self.playerNodeInfo[bulletNode.charID-1].cur_zi_dan_cost, bulletNode.sendAttackFishList[1],bulletNode.charID-1);
		end
	end
end
...
function TableLayer:getDropGoods(goodsInfoList , chairID ,fishPos)
	...
	for k,goodsInfo in pairs(goodsInfoList) do
		if goodsInfo.goodsID > 0 and goodsInfo.goodsCount > 0 and goodsInfo.goodsID ~= GOODS_REDENVELOPES then
			...
			--其他物品加到背包
			local getDropGoodsNode = require("fishing.game.core.getDropGoods").new(goodsInfo)
...
function TableLayer:gameReliefMoney(msg)
	...
	if ret == 0 then
		if self.my then
			...
			local layer = require("fishing.game.core.freeCoinNode").new(2,5,function() globalUnit:setIsRequestReliefMoneyStatue(false); self:sendRequestReliefMoneyResult(); end)			
...
function TableLayer:showBossWarning(fishType)
	...
	if self.specDeath then
		...
		self.specDeath:runAction(cc.Sequence:create(cc.DelayTime:create(3), cc.CallFunc:create(function() 
			if self.specDeath == nil or tolua.isnull(self.specDeath) then
				self.specDeath = nil
			else
...
function TableLayer:createOnlineReward(time)
	...
	-- local size = cc.Director:getInstance():getWinSize();
	-- local layer = require("fishing.game.core.OnlineFreeCoinNode").new(1,time,function() self:reqOnlineReward(); end)			
...
function TableLayer:receiveUserMatchInfo(data)
	...
	if data.UserID == PlatformLogic.loginResult.dwUserID then
		...
		if not layer then
			self._isGameEnd = true;
			layer = require("fishing.game.MatchResultLayer"):create(self);
			layer:setLocalZOrder(101)
...
function TableLayer:showGiftEggsTimer(pos)
	...
	bg:onClick(function(sender)
		...
		local layer = require("fishing.game.core.GiftEggsLayer"):create();
		layer:setLocalZOrder(101)
		self.Sprite_bg:addChild(layer);
	end);
...
function TableLayer:showSpecialRewardLayer(event)
	...
	if self.my then
		if event.flag then
			local layer = require("fishing.game.core.GetRewardLayer"):create(function() luaDump(self.rewardData,"self.rewardData1") self:receiveHitGiftEggsInfo(self.rewardData); end,self.rewardData._usedata);
			layer:setLocalZOrder(1010000);
			self.Sprite_bg:addChild(layer);
		else
			performWithDelay(self, function()
				local layer = require("fishing.game.core.GetRewardLayer"):create(function() luaDump(self.rewardData,"self.rewardData2") self:receiveHitGiftEggsInfo(self.rewardData); end,self.rewardData._usedata);
				layer:setLocalZOrder(1010000);
				self.Sprite_bg:addChild(layer);
			end, 0.6);
		end
...
function TableLayer:showRockMoneyTreeTimer(pos)
	...
	bg:onClick(function(sender)
		...
		local layer = require("fishing.game.core.RockingMoneyTreeLayer"):create();
		layer:setLocalZOrder(101)
		self.Sprite_bg:addChild(layer);
	end);
...
function TableLayer:receiveUserExpandInfo(data)
	...
	if self.redEnvelopesNode then
		...
		local size = cc.Director:getInstance():getWinSize();
		local redEnvelopesNode = require("fishing.game.core.RedEnvelopesNode"):create();
...
function TableLayer:showPrizeLayer()
	...
	local layer = require("fishing.game.core.PrizeLayer"):create();
	layer:setLocalZOrder(101)
	self.Sprite_bg:addChild(layer);
end
...
function TableLayer:showChaoticBeads(catchinfo,pos)
	if self.chaoticBeadFish == nil then
		...
		local seq = cc.Sequence:create(
			...
			cc.CallFunc:create(function()
				if self.chaoticBeadFish and not tolua.isnull(self.chaoticBeadFish) then
					self.chaoticBeadFish:removeSelf();
				end
