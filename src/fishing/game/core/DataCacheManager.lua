...
function DataCacheManager:createBullet(database,paoOption,paoLevel,memberOrder,bSetLockFishSlow)
 ...
 --查找缓存数据 
  ...
  local bullet = require("fishing.game.core.bullet").new(database,paoOption,paoLevel,memberOrder,bSetLockFishSlow);
  bullet.isUse = true;
  bullet:retain();
...
function DataCacheManager:createFish(fishType,path)
  ...
  if (fishType == FishType.FISH_ZHENZHUBEI-1 or fishType == FishType.FISH_XIAOFEIYU-1 or fishType == FishType.FISH_ALILAN-1 or fishType == FishType.FISH_ZHADANFISH-1 or fishType == FishType.FISH_NORMAL_TYPEMAX-1 or fishType == FishType.FISH_WORLDBOSS-1) then
    fish = require("fishing.game.core.SpecialFish").new(fishType);
  elseif fishType >= FishType.FISH_BOSS1-1 and fishType <= FishType.FISH_BOSS4-1 then
    fish = require("fishing.game.core.BossFish").new(fishType);
  else
    fish = require("fishing.game.core.Fish").new(fishType);
  end
...
function DataCacheManager:createFishAnimation(fish ,fishType)
  ...
  if(tempFishType <= FishType.FISH_KIND_COUNT and tempFishType >= 0 )then
    fileName = ("fish_"..tempFishType.."_%d.png")
  else
    --没有替换的鱼
    fileName = "fish_1_%d.png"
...
function DataCacheManager:createCoin(isLiquan,coinType)
  ...
  local coin = require("fishing.game.core.coin").new(isLiquan,coinType);
...
function DataCacheManager:initEffect(effectType,tm)
    ...
    if(effectType == GAMME_CLICK_EFFECT)then
      effect = require("fishing.game.core.clickEffect").new();
    --转轮动画
    elseif(effectType == GAME_ZHUANLUN_EFFECT)then
      effect = require("fishing.game.core.zhuanlunEffect").new();
    --赚翻了动画
    elseif(effectType == GAME_ZHUANFAN_EFFECT)then
      effect = require("fishing.game.core.BigFishCoinArmature").new();
    --捕获大鱼粒子特效
    elseif(effectType == FISH_CATCH_BIG_EFFECT)then
      effect = require("fishing.game.core.bigArmature").new();
    --捕获一般粒子特效
    elseif(effectType == FISH_CATCH_SMALL_EFFECT)then
      effect = require("fishing.game.core.smallArmature").new();
    --全屏
    elseif(effectType == QUAN_PING_EFFECT)then
      effect = require("fishing.game.core.quanPingArmature").new(2);
    --局部
    elseif effectType == JUBU_EFFECT then
    effect = require("fishing.game.core.quanPingArmature").new(1);
    --冰冻
    elseif(effectType == BING_DONG_EFFECT)then
      effect = require("fishing.game.core.bingDongArmature").new();
    --获取金币
    elseif(effectType == COIN_TEXT)then
      effect = require("fishing.game.core.coinText").new();
    elseif(effectType == SHUAI_DAI_LE)then
      effect = require("fishing.game.core.ShuaiDaiLeArmature").new();
    elseif(effectType == HUA_FEI_EFFECT)then
      effect = require("fishing.game.core.huaFeiArmature").new();
    elseif effectType == BING_DONG_LAYER then--冰冻层
      effect = require("fishing.game.core.FrozenLayer").new(tm)
    end
