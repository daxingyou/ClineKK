...
function Fish:updateData()
  ...
  if self._fishType >= FishType.FISH_FENGHUANG and self._fishType <= FishType.FISH_KIND_40 then
    self._fishKing = true;
  elseif self._fishType == FishType.FISH_LONGXIA then--李逵
...
function Fish:createShadow()
  if not self:checkIsSanyanSixi() then
    ...
    local fishShadow = require("fishing.game.core.fishShadow").new(self);
...
function Fish:createSpecFishShadow(isLine)
  ...
  if self:checkIsSanyanSixi() then
    fishShadow = require("fishing.game.core.specialFishShadow").new(self,isLine);
  end
...
function Fish:addFishWidget(size,x)
  ...
  if(not self.fishWidget or tolua.isnull(self.fishWidget))then
    ...
    if size == nil then
      ...
      elseif self._fishType == 34 then
        size.height = size.height*2.2;
        size.width = size.width*1.2;
      elseif self._fishType == 35 then
      ...
      elseif self._fishType == 37 then
        size.height = size.height*1.23;
        size.width = size.width*1.2;
    ...
    if (self._fishType == 28 or self._fishType == 29 or self._fishType == 30) and self.isLine ~= true then
...
function Fish:getFishBoundingBox()
    ...
    elseif self._fishType == 24 then
      self.fishRect =cc.rect(fishRect.width*0.3,fishRect.height*0.2,fishRect.width*0.6,fishRect.height*0.5)
...
function Fish:runSpecialDeath(isMyFireAttackFish,seatID)
  ...
  if self._deadFrameCount > 0 then
    local fileName = "fish_"..self._fishType.."_d%d.png"
...
function Fish:checkIsSanyanSixi()
  ...
  if fishType == FishType.FISH_ZHENZHUBEI or fishType == FishType.FISH_XIAOFEIYU or fishType == FishType.FISH_ALILAN or fishType == FishType.FISH_ZHADANFISH or fishType == FishType.FISH_NORMAL_TYPEMAX or fishType == FishType.FISH_WORLDBOSS then
    return true;
  end
...
function Fish:checkIsBoss()
  if self._fishType == FishType.FISH_BOSS1 or self._fishType == FishType.FISH_BOSS2 or self._fishType == FishType.FISH_BOSS3 or self._fishType == FishType.FISH_BOSS4 then
    return true;
  end
