...
function TableLogic:dealGameMessage(messageHead, object, objectSize)
    ...
    elseif bAssistantID == GameMsg.SUB_S_ANDROID then
    	self:dealGameRootFire(object, objectSize)
     elseif bAssistantID == GameMsg.SUB_S_HIT_FISH then
    	self:dealGameHitFish(object, objectSize)
    end
end
...
function TableLogic:dealGameStationResp(object, objectSize)
	...
	local fishArray = {}
	for k,v in pairs(msg.fish_trace_info) do
		if v.fish_id >0 then
			table.insert(fishArray,v)
		end
	end
	self._callback:refreshNewFish(fishArray,true);
end
...
function TableLogic:dealGameConfig(object, objectSize)
	...
	-- FishModule.m_gameFishConfig.min_bullet_multiple = goldConvert(FishModule.m_gameFishConfig.min_bullet_multiple)
	-- FishModule.m_gameFishConfig.max_bullet_multiple = goldConvert(FishModule.m_gameFishConfig.max_bullet_multiple)
	--刷新玩家炮倍
...
function TableLogic:dealGameUserFire(object,objectSize)
	...
	-- msg.fish_score = goldConvert(msg.fish_score)
...
function TableLogic:dealGameCatchFish(object,objectSize)
	if self._callback.m_bGameStart ~= true then
		return;
	end
...
function TableLogic:dealGameRootFire(object,objectSize)
	local msg = convertToLua(object, GameMsg.CMD_S_Android)

	self._callback:gameRobotDeal(msg)
end

function TableLogic:dealGameHitFish(object,objectSize)
	local msg = convertToLua(object, GameMsg.CMD_S_HitFish)
	luaDump(msg, "dealGameHitFish");
	self._callback:gameHitFish(msg)
end
...
-- 开火
function TableLogic:sendUserFire(bulletKind, angle, bulletMulr, bulletID, lockFishID,lSeatNo)
	...
	msg.angle = angle;
	msg.bullet_mulriple = bulletMulr--goldReconvert(bulletMulr);
	msg.lock_fishid = lockFishID;
	msg.bullet_id = bulletID;
	msg.chair_id = lSeatNo
	-- luaDump(msg,"发送打炮消息")
