...
local szGameName = {"lkpy_0","lkpy_4"}
...
function CreateGoldRoomLayer:initUI()
	...
	if GamesInfoModule:findGameName(40010000) and GamesInfoModule:findGameName(40010004) then
		ret = true
		--模式按钮
	...
	if GamesInfoModule:findGameName(40010004) then
		if not ret then
			self.isRoomMode = 0
			globalUnit:setIsEnterRoomMode(self.isRoomMode)
		end
	...
	if GamesInfoModule:findGameName(40010000) then
		if not ret then
			self.isRoomMode = 1
			globalUnit:setIsEnterRoomMode(self.isRoomMode)
		end
...
function CreateGoldRoomLayer:createItem(tag,name)
	...
	if name ~= "tiyanchang" then
	 ...
	 --    btn:addChild(gold)
	    ...
	    local Roomid = {4,5,6,7}--ruchangNum[k] --最小金币限制
	    ...
	    local minRoomKey = RoomInfoModule:getRoomNeedGold(40010000,Roomid[tag])
...
function CreateGoldRoomLayer:refreshGameList(data)
	...
	if data == 1 then
		if GamesInfoModule:findGameName(40010000) or GamesInfoModule:findGameName(40010004) then
			self:initData()
...
function CreateGoldRoomLayer:enterGame(sender)
	...
	if self.isRoomMode == 1 and globalUnit.iRoomIndex == 1 and #self.roomList > 4 then
		local index = {1}
		for i=5,#self.roomList do
			table.insert(index,i)
		end
		math.randomseed(tostring(os.time()):reverse():sub(1,7))
		local num = math.random(1,#index)
		self.m_selectedRoomID = self.roomList[index[num]].uRoomID
	end
...
function CreateGoldRoomLayer:getRootIndex()
	...
	for k,v in pairs(self.roomList) do
		if RoomInfoModule:isCashRoom(v.uNameID,v.uRoomID) then
			...
			if gold then
				if gold > 0 and gold <= PlatformLogic.loginResult.i64Money then
					targetNodeID = k
				end
			end
		end
