...
function RankInfoLayer:initUI()
	if self.Button_hideOther then
		luaPrint("找到关闭按钮");
		self.Button_hideOther:onClick(function ( ... )
			-- body
			luaPrint("close");
...
function RankInfoLayer:setRankInfo(rankItem,index)
   ...
   luaDump(userInfo,"userInfo")
    if userInfo  then
        ...
        -- HeadManager:requestHttp(100027, Image_otherHead, 10);
         --Image_otherHead:setString(FormotGameNickName(infoTable.userName,nickNameLen));
        end
    end
