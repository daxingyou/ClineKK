...
function TableLogic:loadUsers()
	...
	for i=1,PLAY_COUNT do
	    ...
	    if self._existPlayer[i] == true and user ~= nil then
			...
			--self._callback:setUserMaster(realseatNo, self.m_dwTableOwnerIdx == user.dwUserID);
			--//断线用户显示断线图标
...
function TableLogic:dealUserUpResp(userUp,user,bIsMe)
	...
	luaDump(userUp,"userUp");
	--luaPrint("数据================"..viewseatNo);

	--self._callback:setUserName(viewseatNo," ");
	--self._callback:setUserMoney(viewseatNo, 0);
	self._callback:playerGetReady(viewseatNo, false);
	self._callback:setUserTuoGuan(viewseatNo,false);
...
function TableLogic:gameStationCall(msg)
	self._callback:dealGameStationCall(msg);
end
--下注状态
function TableLogic:gameStationScore(msg)
	self._callback:dealGameStationScore(msg);
end
--游戏状态
...
function TableLogic:I_R_M_GamePoint(msg)
	luaPrint("I_R_M_GamePoint",self._callback.m_bGameStart);
