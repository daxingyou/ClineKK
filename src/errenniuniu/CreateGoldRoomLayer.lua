...
function CreateGoldRoomLayer:initUI()
	...
	local title = ccui.ImageView:create("createRoom/errenpinshiTitle.png")
...
function CreateGoldRoomLayer:createItem(tag)
	...
	local ruchang = {nil,"50.00","200.00","1000.00","5000.00"}
...
function CreateGoldRoomLayer:onClick(sender)
	...
	if tag == 1 then
		...
		self:getParent():addChild(HelpLayer:create(),gameZorder);
	elseif tag == 2 then
		Hall:exitGame()
