...
local CreateRoomHistory = require("yaoyiyao.CreateRoomHistory");
...
function CreateGoldRoomLayer:initUI()
	...
	-- 		luziBg:setPosition(size.width/2,(luziBg:getContentSize().height-10)*(2-i)+5+10*(2-i));
	...
	-- btn:setPosition(btn1:getPositionX()-btn:getContentSize().width/2-40,top:getContentSize().height/2)
	-- btn:setTag(2)
	-- top:addChild(btn)
...
function CreateGoldRoomLayer:showFangjian()
	...
	if VisibleCount == 0 then
		LoadingLayer:createLoading(FontConfig.gFontConfig_22, GBKToUtf8("正在请求游戏列表..."), LOADING):removeTimer()
