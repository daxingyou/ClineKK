...
function TableLogic:sendBankerState(bankState,cancel)--bankState是否上下庄
	...
	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY, GameMsg.ASS_SHANG_ZHUANG, msg, GameMsg.Cr_ShangZhuang)
end
--发送下注信息
function TableLogic:sendPutChipInfo(robotState,chipMoney,chipArea,chipType)--chipType筹码类型 chipArea筹码区域
	msg = {};
	msg.m_bNoteByRobot = robotState;
	msg.money = chipMoney;
	msg.moneytype = chipType;
	msg.type = chipArea;
	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY, GameMsg.ASS_XIA_ZHU, msg, GameMsg.Cr_Packet_XiaZhu)
end
...
function TableLogic:sendXuTouMsg(data)
	msg = {};
	msg.money = data;
	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY, GameMsg.ASS_XIA_ZHU_EX, msg, GameMsg.Cr_Packet_XiaZhuEx)
end
