...
function DataCacheManager:createBullet(database,paoOption,paoLevel,memberOrder,bSetLockFishSlow)
 ...
 --查找缓存数据 
  ...
  local bullet = require("jinchanbuyu.game.core.bullet").new(database,paoOption,paoLevel,memberOrder,bSetLockFishSlow);
  bullet.isUse = true;
  bullet:retain();
...
function DataCacheManager:createFish(fishType,path)
  ...
  --鱼
  local fish = nil;
  if (fishType == FishType.FISH_KIND_64-1 or fishType == FishType.FISH_KIND_65-1 or fishType == FishType.FISH_KIND_66-1) then
    fish = require("jinchanbuyu.game.core.SpecialFish").new(fishType);
  elseif fishType == FishType.FISH_LONGXIA-1 then
    fish = require("jinchanbuyu.game.core.BossFish").new(fishType);
  else
    fish = require("jinchanbuyu.game.core.Fish").new(fishType);
  end
...
function DataCacheManager:createFishAnimation(fish ,fishType)
  ...
  elseif fishType >= FishType.FISH_KIND_67 and fishType <= FishType.FISH_KIND_71 then
    tempFishType = fishType - 64;
    if fishType == FishType.FISH_KIND_71 then
      tempFishType = fishType - 63;
    end
  ...
  elseif fishType == FishType.FISH_HUAFEI7 then
    tempFishType = 20;
  elseif fishType >= FishType.FISH_KIND_64 and fishType <= FishType.FISH_KIND_66 then
    tempFishType = 1;
  ...
  if(tempFishType <= FishType.FISH_KIND_COUNT and tempFishType >= 0 )then
    fileName = ("jcby_fish_"..tempFishType.."_%d.png")
  else
    --没有替换的鱼
    fileName = "jcby_fish_1_%d.png"
...
function DataCacheManager:createCoin(isLiquan,coinType)
  ...
  local coin = require("jinchanbuyu.game.core.coin").new(isLiquan,coinType);
...
function DataCacheManager:initEffect(effectType,tm)
    ...
    if(effectType == GAMME_CLICK_EFFECT)then
      effect = require("jinchanbuyu.game.core.clickEffect").new();
    --转轮动画
    elseif(effectType == GAME_ZHUANLUN_EFFECT)then
      -- effect = require("jinchanbuyu.game.core.zhuanlunEffect").new();
      effect = require("jinchanbuyu.game.core.BigFishEffect").new(1);
    --赚翻了动画
    elseif(effectType == GAME_ZHUANFAN_EFFECT)then
      effect = require("jinchanbuyu.game.core.BigFishCoinArmature").new();
      -- effect = require("jinchanbuyu.game.core.BigFishEffect").new(2);
    --捕获大鱼粒子特效
    elseif(effectType == GAME_CATCH_BOSS_EFFECT)then
      effect = require("jinchanbuyu.game.core.BigFishEffect").new(3);
    --捕获大鱼粒子特效
    elseif(effectType == FISH_CATCH_BIG_EFFECT)then
      effect = require("jinchanbuyu.game.core.bigArmature").new();
    --捕获一般粒子特效
    elseif(effectType == FISH_CATCH_SMALL_EFFECT)then
      effect = require("jinchanbuyu.game.core.smallArmature").new();
    --全屏
    elseif(effectType == QUAN_PING_EFFECT)then
      effect = require("jinchanbuyu.game.core.quanPingArmature").new(2);
    --局部
    elseif effectType == JUBU_EFFECT then
    effect = require("jinchanbuyu.game.core.quanPingArmature").new(1);
    --冰冻
    elseif(effectType == BING_DONG_EFFECT)then
      effect = require("jinchanbuyu.game.core.bingDongArmature").new();
    --获取金币
    elseif(effectType == COIN_TEXT)then
      effect = require("jinchanbuyu.game.core.coinText").new();
    elseif(effectType == SHUAI_DAI_LE)then
      effect = require("jinchanbuyu.game.core.ShuaiDaiLeArmature").new();
    elseif(effectType == HUA_FEI_EFFECT)then
      effect = require("jinchanbuyu.game.core.huaFeiArmature").new();
    elseif effectType == BING_DONG_LAYER then--冰冻层
      effect = require("jinchanbuyu.game.core.FrozenLayer").new(tm)
    end
