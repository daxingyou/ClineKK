...
local TableLogic = require("jinchanbuyu.game.TableLogic");
local ResultLayer = require("jinchanbuyu.game.ResultLayer");
local HelpLayer = require("jinchanbuyu.popView.HelpLayer");
--捕鱼
FishManager = require("jinchanbuyu.game.core.FishManager"):getInstance();
local FishGroup = require("jinchanbuyu.game.core.FishGroup"):getInstance();
local FishMatrix = require("jinchanbuyu.game.core.FishMatrix"):getInstance();
...
function TableLayer:ctor(uNameId, bDeskIndex, bAutoCreate, bMaster)
	...
	display.loadSpriteFrames("jinchanbuyu/fishing/game/gamefish_jcby.plist", "jinchanbuyu/fishing/game/gamefish_jcby.png")
...
function TableLayer:onEnter()
	...
	-- 	player = require("jinchanbuyu.game.core.matchPlayer");
	-- else
		player = require("jinchanbuyu.game.core.player");
	-- end
...
function TableLayer:initGameNormalBtn()
	...
	if self.Button_task then
		...
		--任务换成自动
		display.loadSpriteFrames("jinchanbuyu/fishing/game/gamefish_jcby.plist", "jinchanbuyu/fishing/game/gamefish_jcby.png")
		-- self.Button_task:loadTextures("jcby_zidong.png","jcby_zidong-on.png","jcby_zidong.png",UI_TEX_TYPE_PLIST);
		try({
			function() self.Button_task:loadTextures("jcby_zidong.png","jcby_zidong-on.png","jcby_zidong.png",UI_TEX_TYPE_PLIST) end,
			catch = function(error)
				display.removeSpriteFrames("jinchanbuyu/fishing/game/gamefish_jcby.plist", "jinchanbuyu/fishing/game/gamefish_jcby.png")
				display.loadSpriteFrames("jinchanbuyu/fishing/game/gamefish_jcby.plist", "jinchanbuyu/fishing/game/gamefish_jcby.png")
				self.Button_task:loadTextures("jcby_zidong.png","jcby_zidong-on.png","jcby_zidong.png",UI_TEX_TYPE_PLIST)
			end
		})
	...
	if self.Button_show then
		display.loadSpriteFrames("jinchanbuyu/fishing/game/gamefish_jcby.plist", "jinchanbuyu/fishing/game/gamefish_jcby.png")
		-- self.Button_show:loadTextures("jcby_yincang.png","jcby_yincang-on.png","jcby_yincang.png",UI_TEX_TYPE_PLIST);
		try({
			function() self.Button_show:loadTextures("jcby_yincang.png","jcby_yincang-on.png","jcby_yincang.png",UI_TEX_TYPE_PLIST) end,
			catch = function(error)
				display.removeSpriteFrames("jinchanbuyu/fishing/game/gamefish_jcby.plist", "jinchanbuyu/fishing/game/gamefish_jcby.png")
				display.loadSpriteFrames("jinchanbuyu/fishing/game/gamefish_jcby.plist", "jinchanbuyu/fishing/game/gamefish_jcby.png")
				self.Button_show:loadTextures("jcby_yincang.png","jcby_yincang-on.png","jcby_yincang.png",UI_TEX_TYPE_PLIST)
	...
	self.Button_fast:loadTextures("jcby_fast.png","jcby_fast-on.png","jcby_fast.png",UI_TEX_TYPE_PLIST);
	self.Button_exit:loadTextures("jcby_tuichu.png","jcby_tuichu-on.png","jcby_tuichu.png",UI_TEX_TYPE_PLIST);
	self.Button_multCannon:loadTextures("jcby_jianpao.png","jcby_jianpao-on.png","jcby_jianpao.png",UI_TEX_TYPE_PLIST);
	self.Button_help:loadTextures("jcby_shuoming.png","jcby_shuoming-on.png","jcby_shuoming.png",UI_TEX_TYPE_PLIST);
	self.Button_addCannon:loadTextures("jcby_jiapao.png","jcby_jiapao-on.png","jcby_jiapao.png",UI_TEX_TYPE_PLIST);
	self.Button_lock:loadTextures("jcby_suoding.png","jcby_suoding-on.png","jcby_suoding.png",UI_TEX_TYPE_PLIST);
	self.Button_unlock:loadTextures("jcby_jiesuo.png","jcby_jiesuo-on.png","jcby_jiesuo.png",UI_TEX_TYPE_PLIST);
...
function TableLayer:loadCacheResource()
	local frames = {
					-- "jinchanbuyu/fishing/fish/boss2",
					-- "jinchanbuyu/fishing/fish/boss",
					-- "jinchanbuyu/fishing/fish/boss1",
					"jinchanbuyu/fishing/jcby_fort",
					-- "jinchanbuyu/fishing/fish/jcby_fish_test4",
					-- "jinchanbuyu/fishing/fish/jcby_fish_test3",
					"jinchanbuyu/fishing/fish/jcby_fish_test2",
					"jinchanbuyu/fishing/fish/jcby_fish_test1",
					"jinchanbuyu/fishing/game/gamefish_jcby",
					"jinchanbuyu/fishing/scene/gold_jcby",
					"jinchanbuyu/fishing/scene/jcby_other_gold"
					};
...
function TableLayer:onClickShowBtn(sender)
	...
	if sender:getTag() == 1 then--显示
		...
		sender:setTag(1);
		-- sender:setRotation(45);
		self.Button_show:loadTextures("jcby_show.png","jcby_show-on.png","jcby_show.png",UI_TEX_TYPE_PLIST);
	end
end
...
function TableLayer:onClickAutoCallBack(sender,isPochan)
	...
	display.loadSpriteFrames("fishing/fishing/game/gamefish_jcby.plist", "fishing/fishing/game/gamefish_jcby.png")
	local path = "jcby_zidong"	
	if tag == 0 then  --自动
		tag = 1;
		self:stopAutoFire();
		self:startAutoFire();
...
function TableLayer:onClickDistanceCallBack()
	luaPrint("距离按钮回调函数")
	local ScanLayer =  require("jinchanbuyu.game.ScanLayer");
...
function TableLayer:onClickDistance(sender)
	if self.isShowPlayInfo then
		...
		if userInfo ~= nil then
			local DistanceLayer =  require("jinchanbuyu.game.DistanceLayer");
...
function TableLayer:addPlayer(userInfo)
	...
	if userInfo.dwUserID == PlatformLogic.loginResult.dwUserID then
		...
		if not self:getChildByName("robot") then
			local layer = require("jinchanbuyu.game.RobotManager").new()
...
function TableLayer:addFish(fishType, pathType, fishID, fishElapsedTime,isSpec)
	if fishType < 39 or fishType ==54 or fishType == 55 or (fishType>=63 and fishType<=70) then
		if fishType>=24 and fishType<=29 then
			return
		end
	else
		return
...
-- local fish = self:addFish(41,math.random(1,6000),10);
	...
	-- performWithDelay(self, function() FishMatrix:createFishMatrix(self, 0,{}); end,5);
	...
	if bombFish then
		...
		if msg.chair_id == self.my:getSeatNo() or msg.android_chairid == self.my:getSeatNo() or self.robotLayer.robotList[msg.chair_id] ~= nil  then
			...
			elseif bombFish._fishType >= FishType.FISH_FENGHUANG and bombFish._fishType <= FishType.FISH_KIND_40 then--鱼王
				...
				self.tableLogic:sendCatchSweepFish(msg.chair_id, msg.fish_id, allFishID, #allFishID);
			elseif bombFish._fishType >= FishType.FISH_KIND_67 and bombFish._fishType <= FishType.FISH_KIND_71 then--出奇制胜
				local allFishID = {};
				for fishPoint, fish in pairs(self.fishData) do 
					if fish._fishType >= FishType.FISH_KIND_67 and fish._fishType <= FishType.FISH_KIND_71 and fish:isFishInScreen() then
						allFishID[#allFishID + 1] = fish:getFishID();
					end
				end
				local isHave = false
				for k,v in pairs(allFishID) do
					if v == msg.fish_id then
						isHave = true
					end
				end
				if isHave == false then
					allFishID[#allFishID + 1] = msg.fish_id;
				end
				luaPrint("msg.fish_id = ",msg.fish_id)
				luaDump(allFishID,"allFishID = ")
				self.tableLogic:sendCatchSweepFish(msg.chair_id, msg.fish_id, allFishID, #allFishID);
			else
...
function TableLayer:onCatchBomFish(msg)
	...
	if tarFish and tarFish._fishType == 24 and not self.Sprite_bg:getChildByTag(10001) then
		local layer = require("jinchanbuyu.game.core.quanPingArmatureLayer").new(pos)
		layer:setTag(10001)
...
function TableLayer:getDropGoods(goodsInfoList , chairID ,fishPos)
	...
	for k,goodsInfo in pairs(goodsInfoList) do
		if goodsInfo.goodsID > 0 and goodsInfo.goodsCount > 0 and goodsInfo.goodsID ~= GOODS_REDENVELOPES then
			...
			--其他物品加到背包
			local getDropGoodsNode = require("jinchanbuyu.game.core.getDropGoods").new(goodsInfo)
...
function TableLayer:gameReliefMoney(msg)
	...
	if ret == 0 then
		if self.my then
			...
			local layer = require("jinchanbuyu.game.core.freeCoinNode").new(2,5,function() globalUnit:setIsRequestReliefMoneyStatue(false); self:sendRequestReliefMoneyResult(); end)			
...
function TableLayer:createOnlineReward(time)
	...
	-- local size = cc.Director:getInstance():getWinSize();
	-- local layer = require("jinchanbuyu.game.core.OnlineFreeCoinNode").new(1,time,function() self:reqOnlineReward(); end)			
...
function TableLayer:receiveUserMatchInfo(data)
	...
	if data.UserID == PlatformLogic.loginResult.dwUserID then
		...
		if not layer then
			self._isGameEnd = true;
			layer = require("jinchanbuyu.game.MatchResultLayer"):create(self);
			layer:setLocalZOrder(101)
...
function TableLayer:showGiftEggsTimer(pos)
	...
	bg:onClick(function(sender)
		...
		local layer = require("jinchanbuyu.game.core.GiftEggsLayer"):create();
		layer:setLocalZOrder(101)
		self.Sprite_bg:addChild(layer);
	end);
...
function TableLayer:showSpecialRewardLayer(event)
	...
	if self.my then
		if event.flag then
			local layer = require("jinchanbuyu.game.core.GetRewardLayer"):create(function() luaDump(self.rewardData,"self.rewardData1") self:receiveHitGiftEggsInfo(self.rewardData); end,self.rewardData._usedata);
			layer:setLocalZOrder(1010000);
			self.Sprite_bg:addChild(layer);
		else
			performWithDelay(self, function()
				local layer = require("jinchanbuyu.game.core.GetRewardLayer"):create(function() luaDump(self.rewardData,"self.rewardData2") self:receiveHitGiftEggsInfo(self.rewardData); end,self.rewardData._usedata);
				layer:setLocalZOrder(1010000);
				self.Sprite_bg:addChild(layer);
			end, 0.6);
		end
...
function TableLayer:showRockMoneyTreeTimer(pos)
	...
	bg:onClick(function(sender)
		...
		local layer = require("jinchanbuyu.game.core.RockingMoneyTreeLayer"):create();
		layer:setLocalZOrder(101)
		self.Sprite_bg:addChild(layer);
	end);
...
function TableLayer:receiveUserExpandInfo(data)
	...
	if self.redEnvelopesNode then
		...
		local size = cc.Director:getInstance():getWinSize();
		local redEnvelopesNode = require("jinchanbuyu.game.core.RedEnvelopesNode"):create();
...
function TableLayer:showPrizeLayer()
	...
	local layer = require("jinchanbuyu.game.core.PrizeLayer"):create();
	layer:setLocalZOrder(101)
	self.Sprite_bg:addChild(layer);
end
