...
function CreateGoldRoomLayer:initUI()
	...
	local difenNum = {"1","1","2","5"}
	local ruchangNum = {"30.00","30.00","50.00","100.00"}
    ...
    local Label_name = FontConfig.createWithSystemFont("斗\n地\n主",26 ,cc.c3b(90, 44, 14));
...
function CreateGoldRoomLayer:getRootIndex()
	...
	-- 	end
	-- end
	local gold = RoomInfoModule:getRoomNeedGold(self.m_selectedNameID,self.roomList[2].uRoomID);
	if gold then
		if gold <= PlatformLogic.loginResult.i64Money then
			targetNodeID = 2;
		end
...
function CreateGoldRoomLayer:enterGame(sender)
	...
	GameCreator:setCurrentGame(self.m_selectedNameID)
	local roomNum = {15,21,22,23}
	self.m_selectedRoomID = roomNum[globalUnit.iRoomIndex] --self.roomList[globalUnit.iRoomIndex].uRoomID
