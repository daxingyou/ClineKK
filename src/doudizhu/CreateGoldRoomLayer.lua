...
function CreateGoldRoomLayer:initUI()
	...
	local title = ccui.ImageView:create("createRoom/doudizhuTitle.png")
...
function CreateGoldRoomLayer:createItem(tag)
	local difenNum = {nil,"1.00","0.00","0.00","0.10","2.00","5.00"}
	local ruchang = {nil,"30.00","0.00","0.00","3.00","50.00","100.00"}
	...
	local image;
	if tag == 2 then
		image = ccui.ImageView:create("createRoom/jingdianchang.png")
	elseif tag >=5 then
		image = ccui.ImageView:create("createRoom/buxipai.png")
		btn:loadTextures("createRoom/"..(tag-3)..".png","createRoom/"..(tag-3).."-on.png");
		--添加即将开始
		local btnSize = btn:getContentSize();
		-- local jijiang = ccui.ImageView:create("createRoom/jijiangshangxian.png")
		-- jijiang:setPosition(btnSize.width/2,btnSize.height/2+30);
		-- btn:addChild(jijiang);
	else
		image = ccui.ImageView:create("createRoom/tiyanchang.png")
	end
	...
	if difenNum[tag] then
		...
		-- image:setPosition(size.width*0.5,size.height*0.5)
		-- btn:addChild(image)
...
function CreateGoldRoomLayer:onEnterTransitionFinish()
	...
	-- local layout = ccui.Layout:create()
	-- layout:setContentSize(cc.size(self.listView:getContentSize().width/2-360, self.listView:getContentSize().height))
	-- self.listView:pushBackCustomItem(layout)
...
function CreateGoldRoomLayer:getRootIndex()
	...
	-- 	for k1,v1 in pairs(roomTemp) do
	-- 		if v.uRoomID == v1 then
	-- 			flag = true;
	-- 			break;
	-- 		end
	-- 	end

	-- 	if k ~= #self.roomList and flag then
...
function CreateGoldRoomLayer:showCreateRoomLayer(msg)
	...
	globalUnit.isTryPlay = not RoomInfoModule:isCashRoom(self.m_selectedNameID,self.m_selectedRoomID)
...
function CreateGoldRoomLayer:onClick(sender)
	...
	if tag == 1 then
		...
		display.getRunningScene():addChild(PopUpLayer:create());
	elseif tag == 2 then
		Hall:exitGame()
