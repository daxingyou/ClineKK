...
function TableLogic:dealGameStationResp(object, objectSize)
	...
	if self._gameStatus == GameMsg.GS_FREE then--//等待开始
    	local msg = convertToLua(object,GameMsg.CMD_S_StatusFree);
    	luaDump(msg,"场景消息CMD_S_StatusFree");
    	self:gameStationFree(msg);--空闲状态
	elseif self._gameStatus == GameMsg.GS_SENDCARD then
		local msg = convertToLua(object,GameMsg.CMD_S_StatusSend);
		luaDump(msg,"场景消息CMD_S_StatusSend");
		self._callback:gameStationSend(msg);----叫分状态
	elseif self._gameStatus == GameMsg.GS_PLAYING then
		local msg = convertToLua(object,GameMsg.CMD_S_StatusCall);
		luaDump(msg,"场景消息CMD_S_StatusCall");
		self:gameStationCall(msg);----叫分状态
	-- elseif self._gameStatus == GameMsg.GS_PLAYING then
	-- 	local msg = convertToLua(object,GameMsg.CMD_S_StatusRobNT);
	-- 	luaDump(msg,"场景消息CMD_S_StatusRobNT");
	-- 	self._callback:gameStationRobNt(msg);----叫地主状态
	elseif self._gameStatus == GameMsg.GAME_SCENE_DOUBLE then
		local msg = convertToLua(object,GameMsg.CMD_S_StatusDouble);
		luaDump(msg,"场景消息CMD_S_StatusDouble");
		self._callback:gameStationDouble(msg);----加倍状态
	else
		local msg = convertToLua(object,GameMsg.CMD_S_StatusPlay);
		luaDump(msg,"场景消息CMD_S_StatusPlay");
		self:gameStationPlaying(msg);--游戏进行中
	end
...
function TableLogic:gameStationCall(msg)
	self._callback:gameStationCall(msg)
end

function TableLogic:gameStationPlaying(msg)
	self._callback:gameStationPlaying(msg)
end
...
function TableLogic:I_R_M_GamePoint(msg)
	--self._callback:flushOtherMsg();
end

function TableLogic:dealGameBuymoney(msg)
	self._callback:DealBankInfo(msg);
end
...
function TableLogic:dealUserAgreeResp(agree)
	...
	if 1 == agree.bAgreeGame then
		...
		self._callback:playerGetReady(agree);
...
--用户叫分
function TableLogic:sendCallMsg(cbCallScore)
	local msg = {};
	msg.cbCallScore = cbCallScore;
	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY,GameMsg.SUB_C_CALL_SCORE,msg,GameMsg.CMD_C_CallScore);
end

--用户加倍
function TableLogic:sendDoubleMsg(cbDouble)
	local msg = {};
	msg.cbDouble = cbDouble;
	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY,GameMsg.SUB_C_DOUBLE,msg,GameMsg.CMD_C_Double);
end

--用户出牌
function TableLogic:sendOutCardMsg(outCard)
	local msg = {};
	msg.cbCardData = outCard;
	msg.cbCardCount = #outCard;
	local size = getCSize("BYTE") + #outCard * getCSize("BYTE");
	luaDump(msg,"用户出牌");
	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY,GameMsg.SUB_C_OUT_CARD,msg,GameMsg.CMD_C_OutCard);
end
--用戶不出
function TableLogic:sendPassCardMsg()
	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY,GameMsg.SUB_C_PASS_CARD);
end

--用戶托管(取消托管)
function TableLogic:sendTrustee()
	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY,GameMsg.SUB_C_TRUSTEE);
end

--抢地主
function TableLogic:sendRobNT(bRobNT)
	local msg = {};
	msg.bRobNT = bRobNT;

	RoomLogic:send(RoomMsg.MDM_GM_GAME_NOTIFY,GameMsg.SUB_C_ROB_NT,msg,GameMsg.CMD_C_RobNT);
end
